      ...
      elseif object_keys then
         ...
         if options.pretty then
            ...
            for i, key in ipairs(object_keys) do
               ...
               --hack: test for attimestamp and return the elastic timeseries relevant "@timestamp" key, tk 04/23
               local modkey = tostring(KEYS[i])
               if (key=="attimestamp") then
                  modkey = "\"@timestamp\""
               end
               table.insert(COMBINED_PARTS, string.format(FORMAT, key_indent, modkey, encoded_val))
            end
            ...
            for _, key in ipairs(object_keys) do
               ...
               --hack: test for attimestamp and return the elastic timeseries relevant "@timestamp" key, tk 04/23
               local modkey = tostring(key)
               if (key=="attimestamp") then
                  modkey = "@timestamp"
               end
               local encoded_key = encode_value(self, tostring(modkey), parents, etc, options, indent, true)
