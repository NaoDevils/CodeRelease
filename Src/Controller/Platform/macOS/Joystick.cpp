...
bool Joystick::init()
{
  ...
  while (!deviceId && nextDevice < (unsigned)CFArrayGetCount(devArray))
  {
    ...
    if (deviceId)
    {
      ...
      if (!elemAry)
        return false;
      for (int i = 0; elemAry && !isJoystick && i < (int)CFArrayGetCount(elemAry); ++i)
      {
        IOHIDElementRef elem = (IOHIDElementRef)CFArrayGetValueAtIndex(elemAry, i);
        isJoystick = IOHIDElementGetUsagePage(elem) == kHIDPage_GenericDesktop && (IOHIDElementGetUsage(elem) == kHIDUsage_GD_Joystick || IOHIDElementGetUsage(elem) == kHIDUsage_GD_GamePad);
      }
      if (isJoystick)
      {
        ...
        for (int i = 0; i < (int)CFArrayGetCount(elemAry); ++i)
        {
          ...
          if (elemType == kIOHIDElementTypeInput_Misc || elemType == kIOHIDElementTypeInput_Button || elemType == kIOHIDElementTypeInput_Axis || elemType == kIOHIDElementTypeInput_ScanCodes)
          {
