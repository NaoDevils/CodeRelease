...
{
  if (nullptr != pTheView && (event->type() == QEvent::FocusIn || event->type() == QEvent::Paint || event->type() == QEvent::FocusOut || event->type() == QEvent::LayoutRequest)) // prevent sending events after death
  {
