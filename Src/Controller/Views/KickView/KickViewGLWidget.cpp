...
KickViewGLWidget::KickViewGLWidget(KickView& kickView, KickEngineParameters& parameters, KickViewWidget* parent)
    : QOpenGLWidget(parent), widget(*parent), kickView(kickView), renderer(*kickView.robot->createRenderer()), parameters(parameters), moveDrag(false), moveViewOfViewDragXY(false),
      moveViewOfViewDragXZ(false), moveViewOfViewDragYZ(false), moveViewOfViewDragXP(false), moveViewOfViewDragYP(false), moveViewOfViewDragZP(false)
{
  GLubyte test[17][128] = {
      {0x00,
          0x00,
...
{
  renderer.init(this->context()->shareContext() != nullptr);
  ...
  Vector3f cameraPos(0, -1.f, 0.f), targetPos(0, 0, 0);
...
bool KickViewGLWidget::clickControlPoint(const int& x, const int& y)
{
  ...
  for (int phaseNumber = 0; phaseNumber < parameters.numberOfPhases; phaseNumber++)
  {
      ...
      if (!widget.singleDraw || phaseNumber == widget.selectedPoint.phaseNumber)
      {
        for (int limb = 0; limb < Phase::numOfLimbs; limb++)
        {
          for (unsigned int i = 0; i < Phase::numOfPoints; i++)
          {
            ...
            if (KickViewMath::intersectRayAndBox(vecNear, vecFar, cubePoint, originRot, 15.0, 15.0, 15.0, intersection) && limb != Phase::leftFootRot && limb != Phase::rightFootRot
                && limb != Phase::rightHandRot && limb != Phase::leftHandRot)
            {
    ...
    if ((widget.tra2dWindows || widget.tra1dWindows) && !moveViewOfViewDragXY && !moveViewOfViewDragXZ && !moveViewOfViewDragYZ && !moveViewOfViewDragXP && !moveViewOfViewDragYP && !moveViewOfViewDragZP)
    {
      if (widget.selectedPoint.phaseNumber > -1 && widget.selectedPoint.limb > -1)
      {
        ...
        cpWindow1.resize(Phase::numOfPoints);
        cpWindow2.resize(Phase::numOfPoints);
        cpWindow3.resize(Phase::numOfPoints);
        ...
        float scaleFactorX = mini_window_width / std::max<>(maxXrange - minXrange, 1.0f), scaleFactorX1 = mini_window_height / std::max<>(maxXrange - minXrange, 1.0f),
              scaleFactorY = mini_window_height / std::max<>(maxYrange - minYrange, 1.0f), scaleFactorY1 = mini_window_width / std::max<>(maxYrange - minYrange, 1.0f),
...
void KickViewGLWidget::drawBezierCurves(const int& phaseNumber)
{
  ...
  for (int j = 0; j < Phase::numOfLimbs; j++)
  {
    for (unsigned int i = 0; i < Phase::numOfPoints; i++)
    {
      ...
      {
        if (moveDrag || moveViewOfViewDragYP || moveViewOfViewDragZP || moveViewOfViewDragXP || moveViewOfViewDragXY || moveViewOfViewDragXZ || moveViewOfViewDragYZ)
        {
      ...
      if (j == Phase::leftFootRot)
      {
        ...
        drawPos = RotationMatrix::aroundZ(cubePoint[i][j].z()) * RotationMatrix::aroundX(cubePoint[i][j].x()) * drawPos;
        cubePoint2[i][j] = Vector3f(cubePoint[Phase::numOfPoints - 1][Phase::leftFootTra].x() + drawPos.x(),
            cubePoint[Phase::numOfPoints - 1][Phase::leftFootTra].y() + drawPos.y(),
            cubePoint[Phase::numOfPoints - 1][Phase::leftFootTra].z() + drawPos.z());

        Vector3f drawPos2(150.f, 0.f, 0.f);
        drawPos2 = RotationMatrix::aroundZ(cubePoint[i][j].z()) * RotationMatrix::aroundY(cubePoint[i][j].y()) * drawPos2;
        cubePoint[i][j] = Vector3f(cubePoint[Phase::numOfPoints - 1][Phase::leftFootTra].x() + drawPos2.x(),
            cubePoint[Phase::numOfPoints - 1][Phase::leftFootTra].y() + drawPos2.y(),
            cubePoint[Phase::numOfPoints - 1][Phase::leftFootTra].z() + drawPos2.z());
      }
      ...
      if (j == Phase::rightFootRot)
      {
        Vector3f drawPos(0.f, -150.f, 0.f);
        drawPos = RotationMatrix::aroundZ(cubePoint[i][j].z()) * RotationMatrix::aroundX(cubePoint[i][j].x()) * drawPos;
        cubePoint2[i][j] = Vector3f(cubePoint[Phase::numOfPoints - 1][Phase::rightFootTra].x() + drawPos.x(),
            cubePoint[Phase::numOfPoints - 1][Phase::rightFootTra].y() + drawPos.y(),
            cubePoint[Phase::numOfPoints - 1][Phase::rightFootTra].z() + drawPos.z());

        Vector3f drawPos2(150.f, 0.f, 0.f);
        drawPos2 = RotationMatrix::aroundZ(cubePoint[i][j].z()) * RotationMatrix::aroundY(cubePoint[i][j].y()) * drawPos2;
        cubePoint[i][j] = Vector3f(cubePoint[Phase::numOfPoints - 1][Phase::rightFootTra].x() + drawPos2.x(),
            cubePoint[Phase::numOfPoints - 1][Phase::rightFootTra].y() + drawPos2.y(),
            cubePoint[Phase::numOfPoints - 1][Phase::rightFootTra].z() + drawPos2.z());
      }
      ...
      if (j == Phase::leftHandRot)
      {
        ...
        cubePoint2[i][j] = Vector3f(cubePoint[Phase::numOfPoints - 1][Phase::leftArmTra].x() + drawPos2.x(),
            cubePoint[Phase::numOfPoints - 1][Phase::leftArmTra].y() + drawPos2.y(),
            cubePoint[Phase::numOfPoints - 1][Phase::leftArmTra].z() + drawPos2.z());

        Vector3f lHDrawPos(-kickView.robotDimensions.lowerArmLength, 0., 0.);
        lHDrawPos = RotationMatrix::aroundZ(cubePoint[i][j].z()) * RotationMatrix::aroundY(cubePoint[i][j].y()) * RotationMatrix::aroundX(cubePoint[i][j].x()) * lHDrawPos;
        cubePoint[i][j] = Vector3f(cubePoint[Phase::numOfPoints - 1][Phase::leftArmTra].x() + lHDrawPos.x(),
      ...
      if (j == Phase::rightHandRot)
      {
        Vector3f drawPos2(0.f, -kickView.robotDimensions.lowerArmLength, 0.f);
        drawPos2 = RotationMatrix::aroundX(cubePoint[i][j].x()) * drawPos2;
        cubePoint2[i][j] = Vector3f(cubePoint[Phase::numOfPoints - 1][Phase::rightArmTra].x() + drawPos2.x(),
        ...
        rHDrawPos = RotationMatrix::aroundZ(cubePoint[i][j].z()) * RotationMatrix::aroundY(cubePoint[i][j].y()) * RotationMatrix::aroundX(cubePoint[i][j].x()) * rHDrawPos;
        cubePoint[i][j] = Vector3f(cubePoint[Phase::numOfPoints - 1][Phase::rightArmTra].x() + rHDrawPos.x(),
  ...
  for (float phase = 0.0f; phase < 1.0f; phase += 0.02f)
  {
    ...
    lDrawPos = RotationMatrix::aroundZ(positions[Phase::leftFootRot].z()) * RotationMatrix::aroundX(positions[Phase::leftFootRot].x()) * lDrawPos;
    ...
    lDrawPos2 = RotationMatrix::aroundZ(positions[Phase::leftFootRot].z()) * RotationMatrix::aroundY(positions[Phase::leftFootRot].y()) * lDrawPos2;
    ...
    rDrawPos = RotationMatrix::aroundZ(positions[Phase::rightFootRot].z()) * RotationMatrix::aroundX(positions[Phase::rightFootRot].x()) * rDrawPos;
    ...
    rDrawPos2 = RotationMatrix::aroundZ(positions[Phase::rightFootRot].z()) * RotationMatrix::aroundY(positions[Phase::rightFootRot].y()) * rDrawPos2;
    ...
    Vector3f lHDrawPos(-kickView.robotDimensions.lowerArmLength, 0., 0.);
    lHDrawPos = RotationMatrix::aroundZ(positions[Phase::leftHandRot].z()) * RotationMatrix::aroundY(positions[Phase::leftHandRot].y())
    ...
    Vector3f rHDrawPos(-kickView.robotDimensions.lowerArmLength, 0., 0.);
    rHDrawPos = RotationMatrix::aroundZ(positions[Phase::rightHandRot].z()) * RotationMatrix::aroundY(positions[Phase::rightHandRot].y())
...
{
  Vector3f vec1(point2.x() - point1.x(), point2.y() - point1.y(), point2.z() - point1.z());
  ...
  {
    Vector3f vec(point2.x() - point0.x(), point2.y() - point0.y(), point2.z() - point0.z());
...
void KickViewGLWidget::paintGL()
{
  ...
  Vector3f targetPosTempOffset(targetPosOffset), cameraPosTempOffset(cameraPosOffset);
  ...
  if (widget.tra2dWindows || widget.tra1dWindows || widget.velocityWindows || widget.accelWindows)
  {
    ...
    // TODO: Qt6: renderText is not available. Find proper replacement!
    //if(widget.tra2dWindows)
    //{
    ...
    if (widget.selectedPoint.phaseNumber > -1 && widget.selectedPoint.limb > -1)
    {
      ...
      else
      {
        ...
        for (float phase = 0.f; phase <= 1.f; phase += 0.01f)
        {
          ...
          {
            point.x() = (-3.f + 6.f * phase - 3.f * phase * phase) * p0.x() + 3.f * (2 * phase - 4.f * phase + 3.f * phase * phase) * cpWindow1[0].y()
                + 3.f * (2.f * phase - 3.f * phase * phase) * cpWindow1[1].y() + 3.f * phase * phase * cpWindow1[2].y();
            point.y() = (-3.f + 6.f * phase - 3.f * phase * phase) * p0.y() + 3.f * (2 * phase - 4.f * phase + 3.f * phase * phase) * cpWindow2[0].y()
                + 3.f * (2.f * phase - 3.f * phase * phase) * cpWindow2[1].y() + 3.f * phase * phase * cpWindow2[2].y();
            point.z() = (-3.f + 6.f * phase - 3.f * phase * phase) * p0.z() + 3.f * (2 * phase - 4.f * phase + 3.f * phase * phase) * cpWindow3[0].y()
                + 3.f * (2.f * phase - 3.f * phase * phase) * cpWindow3[1].y() + 3.f * phase * phase * cpWindow3[2].y();
          }
          else
          {
      ...
      if (widget.tra2dWindows)
      {
        draw2dCurves(minXrange, minYrange, maxXrange, maxYrange, scaleFactorX, scaleFactorY, -minXrange, -minYrange, colorX, colorY, -mini_window_height, cpWindow1, pWindow1);
        draw2dCurves(minXrange, minZrange, maxXrange, maxZrange, scaleFactorX, scaleFactorZ, -minXrange, -minZrange, colorX, colorZ, -mini_window_height * 2 - offsetBetween, cpWindow2, pWindow2);
        draw2dCurves(minYrange, minZrange, maxYrange, maxZrange, scaleFactorY1, scaleFactorZ, -minYrange, -minZrange, colorY, colorZ, -mini_window_height * 3 - offsetBetween * 2, cpWindow3, pWindow3);
      }
      else
      {
        float colorPhase[] = {1.0f, 1.0f, 1.0f, 1.0f};
        draw2dCurves(0.0f, minXrange, 1.0f, maxXrange, mini_window_width, scaleFactorX1, 0.0f, -minXrange, colorPhase, colorX, -mini_window_height, cpWindow1, pWindow1);
        draw2dCurves(0.0f, minYrange, 1.0f, maxYrange, mini_window_width, scaleFactorY, 0.0f, -minYrange, colorPhase, colorY, -mini_window_height * 2 - offsetBetween, cpWindow2, pWindow2);
        draw2dCurves(0.0f, minZrange, 1.0f, maxZrange, mini_window_width, scaleFactorZ, 0.0f, -minZrange, colorPhase, colorZ, -mini_window_height * 3 - offsetBetween * 2, cpWindow3, pWindow3);
      }
    }
    glMatrixMode(GL_PROJECTION);
    glPopMatrix();
...
void KickViewGLWidget::draw2dCurves(const float& minA,
    const float& minB,
    const float& maxA,
    const float& maxB,
...
{
  renderer.zoom(event->angleDelta().y(), -1, -1);
  update();
  event->accept();
  return;
}
...
void KickViewGLWidget::mouseMoveEvent(QMouseEvent* event)
{
  ...
  actualX = event->position().x();
  actualY = event->position().y();

  if ((moveDrag || moveViewOfViewDragXY || moveViewOfViewDragXZ || moveViewOfViewDragYZ || moveViewOfViewDragXP || moveViewOfViewDragYP || moveViewOfViewDragZP)
      && widget.selectedPoint.limb > -1)
  {
    Vector3f vecFar, vecNear;
    gluUnProjectClick(event->position().x(), event->position().y(), vecFar, vecNear);
    ...
    else
    {
      ...
      {
        windowVec = Vector3f(
            ((event->position().x() - (width - 20.f - width / 3.0f)) / scaleFactorX) + minXrange, ((event->position().y() - mini_window_height - 20.0f) / -scaleFactorY) + minYrange, 0.0);
        p.z() = 0.f;
      }
      if (moveViewOfViewDragXZ)
      {
        windowVec = Vector3f(
            ((event->position().x() - (width - 20.f - width / 3.0f)) / scaleFactorX) + minXrange, 0.0, ((event->position().y() - mini_window_height * 2 - 55.0f) / -scaleFactorZ) + minZrange);
        p.y() = 0.f;
      }
      if (moveViewOfViewDragYZ)
      {
        windowVec = Vector3f(
            0.0, ((event->position().x() - (width - 20.f - width / 3.0f)) / scaleFactorY1) + minYrange, ((event->position().y() - mini_window_height * 3 - 90.0f) / -scaleFactorZ) + minZrange);
        p.x() = 0.f;
      }
      if (moveViewOfViewDragXP)
      {
        windowVec = Vector3f(((event->position().y() - mini_window_height - 20.0f) / -scaleFactorX1) + minXrange, 0.0, 0.0);
        p.y() = 0.f;
        p.z() = 0.f;
      }
      if (moveViewOfViewDragYP)
      {
        windowVec = Vector3f(0.0, ((event->position().y() - mini_window_height * 2 - 55.0f) / -scaleFactorY) + minYrange, 0.0);
        p.x() = 0.f;
        p.z() = 0.f;
      }
      if (moveViewOfViewDragZP)
      {
        windowVec = Vector3f(0.0, 0.0, ((event->position().y() - mini_window_height * 3 - 90.0f) / -scaleFactorZ) + minZrange);
        p.y() = 0.f;
        p.x() = 0.f;
      }
    ...
    case 0:
      ...
      {
        float factor = (float)parameters.phaseParameters[widget.selectedPoint.phaseNumber - 1].duration / (float)parameters.phaseParameters[widget.selectedPoint.phaseNumber].duration;
  ...
  {
    if (renderer.moveDrag(event->position().x(), event->position().y(), QApplication::keyboardModifiers() & Qt::ShiftModifier ? SimRobotCore2::Renderer::dragRotate : SimRobotCore2::Renderer::dragNormal))
    {
      event->accept();
      if (widget.selectedPoint.phaseNumber > -1)
        widget.fillModelWithPhaseData(widget.selectedPoint.phaseNumber);
...
void KickViewGLWidget::mousePressEvent(QMouseEvent* event)
{
  ...
  {
    if (clickControlPoint(event->position().x(), event->position().y()))
    {
    ...
    if (renderer.startDrag(event->position().x(), event->position().y(), QApplication::keyboardModifiers() & Qt::ShiftModifier ? SimRobotCore2::Renderer::dragRotate : SimRobotCore2::Renderer::dragNormal))
    {
      event->accept();
      update();
    }
...
void KickViewGLWidget::mouseReleaseEvent(QMouseEvent* event)
{
  ...
  {
    if (renderer.releaseDrag(event->position().x(), event->position().y()))
    {
      event->accept();
      update();
    }
