...
void DebugDrawing::arrow(Vector2f start, Vector2f end, Drawings::PenStyle penStyle, int width, ColorRGBA color)
{
...
void DebugDrawing::polygon(const Vector2i* points, int nCount, int width, Drawings::PenStyle penStyle, ColorRGBA penColor, Drawings::BrushStyle brushStyle, ColorRGBA brushColor)
{
  Polygon element;
...
bool DebugDrawing::addShapeFromQueue(InMessage& message, Drawings::ShapeType shapeType)
{
  ...
  case Drawings::polygon:
  {
    ...
    this->polygon(points, numberOfPoints, penWidth, (Drawings::PenStyle)penStyle, penColor, (Drawings::BrushStyle)brushStyle, brushColor);
    delete[] points;
    break;
  }
...
In& operator>>(In& stream, DebugDrawing& debugDrawing)
{
  debugDrawing.reset();
  int size;
  stream >> debugDrawing.typeOfDrawing >> debugDrawing.timeStamp >> debugDrawing.firstTip >> size;
...
{
  stream << debugDrawing.typeOfDrawing << debugDrawing.timeStamp << debugDrawing.firstTip << debugDrawing.usedSize;
