...
public:
  static void drawInGrid(const std::vector<Vector2f>& draw_targets, const std::vector<float>& draw_scores, const FieldDimensions& theFieldDimensions)
  {
    COMPLEX_DRAWING(DRAW_EXECUTABLE_KICKS_IN_GRID)
    {
      ...
      {
        const int draw_index = HeatMapFunctions::fieldToIndex(draw_target, theFieldDimensions);
      ...
      {
        const Vector2f draw_target = HeatMapFunctions::indexToField(unique_draw_indexes[i], theFieldDimensions);
