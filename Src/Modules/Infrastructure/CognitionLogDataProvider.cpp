...
CognitionLogDataProvider::CognitionLogDataProvider()
    : frameDataComplete(false), lowFrameRateImage(nullptr), lowFrameRateImageUpper(nullptr), sequenceImage(nullptr), sequenceImageUpper(nullptr)
{
  theInstance = this;
}
...
void CognitionLogDataProvider::update(Image& image)
{
  ...
  {
    const CameraInfo& info = Blackboard::get<CameraInfo>();
    const FrameInfo& frameInfo = Blackboard::get<FrameInfo>();
    if (lowFrameRateImage)
    {
      if (lowFrameRateImage->imageUpdated)
      {
        lastImages = lowFrameRateImage->image;
        lastImages.imageSource = ImageSource::lowFrameRateImage;
      }
    ...
    else if (sequenceImage)
    {
      if (sequenceImage->noInSequence > 0)
      {
        ...
        lastImages.imageSource = ImageSource::sequenceImage;
      }
      else if (image.timeStamp != frameInfo.time)
        lastImages = image;
  ...
  IMAGE3D("representation:Image", distance, 0, 0, 0, 0, 0, distance * theCameraInfo.width / theCameraInfo.focalLength, distance * theCameraInfo.height / theCameraInfo.focalLength, image);
}
...
void CognitionLogDataProvider::update(ImageUpper& imageUpper)
{
  if (SystemCall::getMode() == SystemCall::logfileReplay)
  {
    const CameraInfoUpper& info = Blackboard::get<CameraInfoUpper>();
    const FrameInfo& frameInfo = Blackboard::get<FrameInfo>();
    if (lowFrameRateImageUpper)
    {
  ...
  IMAGE3D("representation:ImageUpper",
      distance,
      0,
      0,
      0,
...
void CognitionLogDataProvider::update(ImageCoordinateSystem& imageCoordinateSystem)
{
  ...
  COMPLEX_IMAGE(corrected)
  {
    ...
    {
      const Image& image = Blackboard::get<Image>();
...
void CognitionLogDataProvider::update(ImageCoordinateSystemUpper& imageCoordinateSystem)
{
  ...
  COMPLEX_IMAGE(correctedUpper)
  {
    ...
    {
      const ImageUpper& image = Blackboard::get<ImageUpper>();
...
void CognitionLogDataProvider::update(RobotPoseHypotheses& robotPoseHypotheses)
{
  if (robotPoseHypothesesCompressed)
    robotPoseHypotheses = static_cast<RobotPoseHypotheses>(*robotPoseHypothesesCompressed);
  else
...
bool CognitionLogDataProvider::handleMessage2(InMessage& message)
{
  ...
  case idJPEGImageUpper:
    if (Blackboard::getInstance().exists("ImageUpper"))
    {
      JPEGImageUpper jpegImage;
      message.bin >> jpegImage;
      jpegImage.toImage((Image&)Blackboard::getInstance()["ImageUpper"]);
    }
    return true;
