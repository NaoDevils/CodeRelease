...
public: // constructors
  ...
  template <typename OtherDerived> RotationState(const Eigen::MatrixBase<OtherDerived>& other) : Eigen::Matrix<double, dim, 1>(other) {}

  // This method allows you to assign Eigen expressions to MyVectorType
  template <typename OtherDerived> RotationState& operator=(const Eigen::MatrixBase<OtherDerived>& other)
  {
...
public: // accessors
  Eigen::Block<Eigen::Matrix<double, dim, 1>, 3, 1> rotation() { return Eigen::Block<Eigen::Matrix<double, dim, 1>, 3, 1>(this->derived(), 0, 0); }

  const Eigen::Block<const Eigen::Matrix<double, dim, 1>, 3, 1> rotation() const { return Eigen::Block<const Eigen::Matrix<double, dim, 1>, 3, 1>(this->derived(), 0, 0); }

  Eigen::Block<Eigen::Matrix<double, dim, 1>, 3, 1> rotational_velocity() { return Eigen::Block<Eigen::Matrix<double, dim, 1>, 3, 1>(this->derived(), 3, 0); }
...
public: // define arithmetics
  // unary operator
  RotationState operator-() const { return this->Eigen::Matrix<double, dim, 1>::operator-(); }
...
public:
  ...
  void predict(const Eigen::Vector3d& /*u*/, const double dt)
  {
    ...
    rot_vel_mat << 1, -rot_vel(2), rot_vel(1), rot_vel(2), 1, -rot_vel(0), -rot_vel(1), rot_vel(0), 1;
...
template <class M1, int dim, int dim_cov = dim> class State : public Eigen::Matrix<double, dim, 1>
{
public: // constructors
  // This constructor allows you to construct MyVectorType from Eigen expressions
  template <typename OtherDerived> State(const Eigen::MatrixBase<OtherDerived>& other) : Eigen::Matrix<double, dim, 1>(other) {}

  // inital state (zero rotation, zero angular velocity)
  State() : Eigen::Matrix<double, dim, 1>(Eigen::Matrix<double, dim, 1>::Zero()) {}

  // This method allows you to assign Eigen expressions to MyVectorType
  template <typename OtherDerived> State& operator=(const Eigen::MatrixBase<OtherDerived>& other)
  {
    this->Eigen::Matrix<double, dim, 1>::operator=(other);
    return *this;
  }
...
public: // accessors
  Eigen::Block<Eigen::Matrix<double, dim, 1>> acceleration() { return Eigen::Block<Eigen::Matrix<double, dim, 1>>(this->derived(), 0, 0, 3, 1); }

  const Eigen::Block<const Eigen::Matrix<double, dim, 1>> acceleration() const { return Eigen::Block<const Eigen::Matrix<double, dim, 1>>(this->derived(), 0, 0, 3, 1); }
...
public:
  ...
  // state to measurement transformation function
  // HACK: add return type as parameter to enable overloading...
  M1 asMeasurement(const M1& /*z*/) const { return acceleration(); }
