 ...
 *
 * Declaration of class MultiKalmanModelAngle.
...
/**
  ...
  * as final model. Thus the \c MultiKalmanModelAngle can deal with jumping percepts
  ...
  * \tparam towardsNoOfModels Defines to how many hypotheses the MultiKalmanModelAngle
  *                           converges to.
  */
template <typename hypothesis_t, bool towardsOneModel>
STREAMABLE(MultiKalmanModelAngle,
     ...
     */
    void motionUpdate(unsigned currentTimestamp);
     ...
     */
    hypothesis_t* findNearestHypothesis(Vector2a& angleDiff, float& distance, const Vector2f& measuredPosition);
  ...
  private:
    ...
    /**
     * Checks if the indices of the best hypotheses were already computed (in the current
     * frame). If not \c updateBestHypothesis() is called before returning the indices.
     * \return The indices of the best hypotheses or <tt>static_cast<size_t>(-1)</tt> if no
     *         hypothesis exists.
     */
  ...
  public:
    ...
    void updateMaxMeasurementNoise(double maxMeasurementNoise, bool onlyBestHypothesis);

    void hypothesisKicked(bool longkick);

    void stopBestHypothesis();
     ...
     */
    void cleanUpHypothesesSimilarToBestOne(const Vector2a& mergeAngleDiff);
    ...
    /**
     * Add a copy of the given hypothesis to this \c MultiKalmanModelAngle.
    ...
    /**
     * Add the given hypothesis to this \c MultiKalmanModelAngle.
     * \warning This should be treated with caution! Only add a hypothesis if
     *          you are sure you know what you are doing.
     * \param [in] newHypothesis The hypothesis to add.
     */
     ...
     * \c MultiKalmanModelAngle. This make information about the hypotheses set
...
#include "MultiKalmanModelAngle_impl.h"
