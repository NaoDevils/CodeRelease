 ...
 */
class KalmanPositionHypothesis : public Streamable
{
public:
...
public:
   ...
   */
  std::size_t perceptsPerSecond() const;
  /**
   * Return the <tt>percepts per second</tt> value tracked over
   ...
   * \return Percepts per second.
   */
  std::size_t perceptsPerSecond(unsigned currentTimestamp);
  /**
   ...
   */
  void updateValidity(std::size_t maxPerceptsPerSecond, float weightOfPreviousValidity);
...
private:
  ...
  PerceptsPerSecond<1000> m_perceptsPerSecond;
