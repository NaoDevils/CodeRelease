...
void KalmanRobotMapProvider::update(RemoteRobotMap& remoteRobotMap)
{
  execute();
  (RobotMap&)remoteRobotMap = m_robotMap;
}
...
void KalmanRobotMapProvider::execute()
{
  ...
  m_multiKalmanRobotMap.updateRobotTypes(parameters.validity.Validity_minPerceptsPerSecond,
                                         parameters.validity.Validity_maxPerceptsPerSecond,
                                         theFrameInfo.time);

  // Remove hypotheses with too small validities and those outside the field.
  m_multiKalmanRobotMap.cleanUpHypothesesLowValidity(
    parameters.CleanUpHypotheses_belowValidity, false);
  if (parameters.CleanUpHypotheses_outsideField)
  {
    m_multiKalmanRobotMap.cleanUpHypothesesOutsideField(
...
void KalmanRobotMapProvider::sensorUpdate()
{
  ...
  m_multiKalmanRobotMap.updateValidity(
    parameters.validity.Validity_maxPerceptsPerSecond,
    parameters.validity.goodValidity(),
    parameters.validity.Validity_weightOfPreviousValidity,
    parameters.validity.Validity_weightOfPreviousValidity_goodHypotheses);
}
  ...
  for (const Teammate& teammate: theTeammateData.teammates)
  {
    ...
    if (teammate.status == Teammate::FULLY_ACTIVE)
    {
      for (const RobotEstimate& robot: teammate.robotsPercept.robots)
      {
        ...
        // (Robot height estimated by 500 mm)
        float verticalAnglePercept = atan2f(robot.distance, 500.f);
        float verticalAngleThisRobot = atan2f(Geometry::distance(teammate.pose.translation, theRobotPose.translation), 500.f);
        float verticalAngleDiff = Geometry::absDifferenceBetweenTwoAngles(verticalAnglePercept, verticalAngleThisRobot);
        // TODO: make params?
        if (dist >= 500.f &&
            (horizontalAngleDiff >= 0.5f || verticalAngleDiff >= 0.25f))
        {
          sensorUpdateSingle(percept,
                             robot.distance,
                             robot.robotType,
...
void KalmanRobotMapProvider::sensorUpdateSingle(const Vector2f& position,
                                                ...
                                                int playerNumber)
{
  ...
  MultiKalmanRobotMapHypothesis::TeammateInfo teammate(perceptValidity, timeStamp);
  
  // Run sensor update with global ball position.
  m_multiKalmanRobotMap.sensorUpdate(
    position,
    distanceToRobot,
...
void KalmanRobotMapProvider::generateRemoteRobotMap()
{
  ...
  for (size_t i = 0; i < m_multiKalmanRobotMap.size(); i++)
  {
    ...
    {
      updateRobotMapEntry(m_multiKalmanRobotMap[i], m_robotMap.robots[i]);
    }
  }
}
...
void KalmanRobotMapProvider::plot() const
{
  ...
  PLOT("module:KalmanRobotMapProvider:RobotType_opponent", MultiKalmanRobotMapHypothesis::ROBOT_TYPE_VALIDITY_OPPONENT);
  PLOT("module:KalmanRobotMapProvider:RobotType_teammate", MultiKalmanRobotMapHypothesis::ROBOT_TYPE_VALIDITY_TEAMMATE);
