...
void LineMatcher::execute(LineMatchingResult& theLineMatchingResult)
{
  ...
  for(auto i : theCLIPFieldLinesPercept.lines)
  {
    ...
    theLineMatchingResult.observations.push_back(obs);
  }
...
double LineMatcher::determineMainDirection()
{
  ...
  int accumulatorCount[9];
  double directionAccumulator[9];
  for (int i=0; i<9; i++)
  {
    accumulatorCount[i] = 0;
  ...
  for (auto &i : theCLIPFieldLinesPercept.lines)
  {
    double direction = (i.endOnField - i.startOnField).angle();
    //direction = (direction + pi2) % pi_2; // so it is positive and in the first quadrant
    // shitty C++ does not know modulo for non-integers...
    double temp = direction + pi2;                // so it is positive...
    direction = temp - (int)(temp / pi_2) * pi_2; // ...and in the first quadrant

    int index = (int)(direction / tenDegrees); // [0..8]
...
void LineMatcher::buildLineClusters(LineMatchingResult& theLineMatchingResult)
{
  ...
  // for each FieldLine, find out into which cluster it belongs
  int index=0;
  for(std::vector<LineMatchingResult::FieldLine>::const_iterator i = theLineMatchingResult.observations.begin(); i != theLineMatchingResult.observations.end(); ++i)
  {
    ...
    if (direction < pi_4 || direction > pi3_4)
    {
      ...
      poseForLineTransformation.rotation = (float)(- mainDirection);
      observationMapToInternalMainDirectionClass[index] = true;
    }
    else
    {
      linesCluster = &lines90DegreeToMainDirection;
      poseForLineTransformation.rotation = (float)( - mainDirection - pi_2);
    ...
    int matchIndex = 0;
    for(std::vector<AbstractLine>::iterator j = linesCluster->begin(); j != linesCluster->end(); ++j)
    {
  ...
  int counter = 0;
  for(std::vector<AbstractLine>::iterator i = linesInMainDirection.begin(); i != linesInMainDirection.end(); ++i)
  {
  ...
  counter = 0;
  for(std::vector<AbstractLine>::iterator i = lines90DegreeToMainDirection.begin(); i != lines90DegreeToMainDirection.end(); ++i)
  {
...
{
  tempLines.clear();
  for(std::vector<AbstractLine>::const_iterator i = linesInMainDirection.begin(); i != linesInMainDirection.end(); ++i)
  {
  ...
  lines90DegreeToMainDirection.clear();
  for(std::vector<AbstractLine>::const_iterator i = tempLines.begin(); i != tempLines.end(); ++i)
  {
...
bool LineMatcher::checkGeometricValidityOfCorrespondences(int totalNumberOfObservedLines, int &conflictingPosition, Pose2f& poseHypothesis)
{
  ...
  poseHypothesis.translation.x() = (float)(yLineObserved.offset - yLineOnField.offset);
  poseHypothesis.translation.y() = (float)(xLineOnField.offset - xLineObserved.offset);
...
bool LineMatcher::checkGeometricValidityOfCorrespondencesForIntervals(int totalNumberOfObservedLines, int &conflictingPosition, Pose2f& poseIntervalHypothesisStart, Pose2f& poseIntervalHypothesisEnd)
{
  ...
  {
    poseIntervalHypothesisStart.translation.y() = (float)positionPerpendicularToLineDirection;
    poseIntervalHypothesisEnd.translation.y() = (float)positionPerpendicularToLineDirection;
    poseIntervalHypothesisStart.translation.x() = (float)minPositionInLineDirection;
    poseIntervalHypothesisEnd.translation.x() = (float)maxPositionInLineDirection;
  }
  else
  {
    poseIntervalHypothesisStart.translation.x() = (float)-positionPerpendicularToLineDirection;
    poseIntervalHypothesisEnd.translation.x() = (float)-positionPerpendicularToLineDirection;
    poseIntervalHypothesisStart.translation.y() = (float)minPositionInLineDirection;
    poseIntervalHypothesisEnd.translation.y() = (float)maxPositionInLineDirection;
  }
