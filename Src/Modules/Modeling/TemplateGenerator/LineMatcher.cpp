...
bool LineMatcher::checkGeometricValidityOfCorrespondences(int totalNumberOfObservedLines, int &conflictingPosition, Pose2f& poseHypothesis)
{
	...
	// Plausability check if yLine is observed behind robot
	if (yLineObserved.offset < 0)
	{
		// Check if line starts on one side but ends on the other side of the robot
		// Still allow lines observed behind robot but one side only (look over shoulder)
		if (yLineObserved.min < 0 && yLineObserved.max > 0)
			return false;
		if (yLineObserved.min > 0 && yLineObserved.max < 0)
			return false;
	}
