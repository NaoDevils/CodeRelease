...
WhistleDetectorPeak::WhistleDetectorPeak()
{
    ringPos = 0;
...
void WhistleDetectorPeak::update(WhistleDortmund &whistle)
{ 
  ...
  MODIFY("module:WhistleDetector:ATTACK_TIMEOUT_MS", ATTACK_TIMEOUT_MS);
  MODIFY("module:WhistleDetector:PEAK_DIFF", PEAK_DIFF);
  ...
  MODIFY("module:WhistleDetector:USE_NUTTALL_WINDOWING", USE_NUTTALL_WINDOWING);
    ...
    if (theAudioDataDortmund.isValid)
    {
        ...
        while (audioDataPos < theAudioDataDortmund.samples.size())
        {
            ...
            if (samplesLeft == 0)
            {
                ...
                if (amplitudes[peakPos] >= currMinAmp && std::abs(peakPos - lastPeakPos) < PEAK_DIFF)
                {
    ...
    COMPLEX_IMAGE(FFT)
    {
        ...
        for (int x = min_x; x <= max_x; x++)
            for (int y = static_cast<int>(currMinAmp); y <= DEBUG_HEIGHT; y++)
...
*/
void WhistleDetectorPeak::kissFFT(kiss_fft_cpx in[], kiss_fft_cpx out[])
{
    kiss_fft_cfg cfg;
...
MAKE_MODULE(WhistleDetectorPeak, modeling)
