...
#include "SelfLocator2016.h"

#include "Tools/Math/GaussianDistribution2D.h"

/*---------------------------- class SelfLocator2016 PUBLIC methods ------------------------------*/
...
namespace
{
  ...
  bool arePosesCloseToEachOther(const Pose2f &p1, const Pose2f &p2, const SelfLocator2016Parameters &parameters)
  {
...
SelfLocator2016::SelfLocator2016() :
...
SelfLocator2016::~SelfLocator2016()
{
  PoseHypothesis2016::cleanup();
}

void SelfLocator2016::update(RobotPose& robotPose)
{
  executeCommonCode();

  const PoseHypothesis2016 &bestHyp = getBestHypothesis();
...
void SelfLocator2016::update(SideConfidence& confidence)
{
  executeCommonCode();
...
void SelfLocator2016::update(RobotPoseHypothesis& robotPoseHypothesis)
{
  executeCommonCode();

  const PoseHypothesis2016 &bestRobotPoseHypothesis = getBestHypothesis();
...
void SelfLocator2016::update(RobotPoseHypotheses& robotPoseHypotheses)
{
  executeCommonCode();
...
void SelfLocator2016::predictHypotheses()
{
...
void SelfLocator2016::updateHypothesesPositionConfidence()
{
  bool updateSpherical, updateInfiniteLines, updateWeighted;
  for (auto hyp : poseHypotheses)
  {
    updateSpherical = hyp->updatePositionConfidenceWithLocalFeaturePerceptionsSpherical(theLineMatchingResult, theCLIPCenterCirclePercept, theCLIPGoalPercept, thePenaltyCrossPercept, theFieldDimensions, theCameraMatrix, theCameraMatrixUpper, parameters);
    updateInfiniteLines = hyp->updatePositionConfidenceWithLocalFeaturePerceptionsInfiniteLines(theLineMatchingResult, theCLIPCenterCirclePercept, theFieldDimensions, parameters);
    updateWeighted = hyp->updatePositionConfidenceWithLocalFeaturePerceptionsWeighted(theCLIPFieldLinesPercept, theCLIPCenterCirclePercept, theCLIPGoalPercept, thePenaltyCrossPercept, theFieldDimensions, parameters);
...
void SelfLocator2016::updateHypothesesState()
{
  const PoseHypothesis2016 &ph = getBestHypothesis();
  Pose2f pose;
  ...
  PoseHypotheses2016::iterator it = poseHypotheses.begin();
  while (it != poseHypotheses.end())
  {
    PoseHypothesis2016 *hypothesis = *it;
    ...
    {
      OUTPUT_TEXT("module:SelfLocator2016:updateState produced impossible state -> deleting hypothesis");
    ...
    else
    {
      if (parameters.displayWarnings)
        OUTPUT_TEXT("module:SelfLocator2016:updateState produced state that is outside of carpet -> deleting hypothesis");
      it = poseHypotheses.erase(it);
    }
  }
  if (poseHypotheses.empty())
  {
    OUTPUT_TEXT("module:SelfLocator2016:updateState -> no reasonable hypothesis left!");
    // re-add best one from before
    poseHypotheses.push_back(new PoseHypothesis2016(pose, pc, sc, theFrameInfo.time, parameters));
  }
}

void SelfLocator2016::updateHypothesesSymmetryConfidence()
{
  for (auto hypothesis : poseHypotheses)
  {
    if (theGameInfo.state == STATE_PLAYING && parameters.symmetryUpdate_updateWithRemoteModels) // no ball on the field in READY! (which is the strongest symmetry indicator)
    {
    ...
    else
    {
      ...
      hypothesis->resetSymmetryState(SideConfidence::CONFIDENT);
    }
...
void SelfLocator2016::executeCommonCode()
{
  if (!initialized)
  {
...
void SelfLocator2016::normalizeWeights()
{
...
void SelfLocator2016::pruneHypothesesGoalie()
{
  // Let's simply assume that the keeper will never be in the opponent half!  :)
  PoseHypotheses2016::iterator i = poseHypotheses.begin();
  if (theBehaviorData.role == BehaviorData::keeper)
  {
...
void SelfLocator2016::pruneHypothesesOutsideCarpet()
{
  for (PoseHypotheses2016::iterator it = poseHypotheses.begin();
...
void SelfLocator2016::pruneHypotheses()
{
  if (poseHypotheses.empty())
    return;
  ...
  const PoseHypothesis2016 &bestHypothesis = getBestHypothesis();
  ...
  {
    PoseHypotheses2016::iterator worst = poseHypotheses.begin();
...
bool SelfLocator2016::hasSymmetryBeenLostForBestHypotheses()
{
  const PoseHypothesis2016 &bestHyp = getBestHypothesis();
  return hasSymmetryBeenLost(bestHyp);
}

bool SelfLocator2016::hasSymmetryBeenLost(const PoseHypothesis2016& hypotheses)
{
...
bool SelfLocator2016::hasPositionTrackingFailed()
{
...
bool SelfLocator2016::hasPositionBeenFoundAfterLoss()
{
  if (poseHypotheses.empty())
    return false;
...
void SelfLocator2016::evaluateLocalizationState()
{
...
void SelfLocator2016::handleFallDown()
{
  const PoseHypothesis2016 &bestHyp = getBestHypothesis();
  Pose2f p;
...
void SelfLocator2016::handleGettingUpAfterFallDown()
{
  // if we fell on the side, we might end up with +/- 90° orientation errors,
  // so add those hypotheses...
  std::vector<PoseHypothesis2016*> additionalHypotheses;
  for (auto hypothesis : poseHypotheses)
  {
    hypothesis->scalePositionConfidence(parameters.positionConfidence_scaleFactorAfterFallDown); // = 0.75
    Pose2f pose;
    hypothesis->getRobotPose(pose);
    for (unsigned int j = 1; j <= noAdditionalHypothesisAfterFallDown; ++j)
    {
      PoseHypothesis2016* p1 = new PoseHypothesis2016(*hypothesis); // copy Hypotheses

      const float rot = pi2 * float(j) / (noAdditionalHypothesisAfterFallDown + 1);
      const float scale = 1 - std::abs(Angle::normalize(rot)) / pi;

      const float confidenceScaleMin = 1 - parameters.positionConfidence_scaleFactorAfterFallDown;
      const float confidenceScaleMax = parameters.positionConfidence_scaleFactorAfterFallDown;
...
void SelfLocator2016::handleUnPenalized()
{
...
void SelfLocator2016::handleSetState()
{
...
void SelfLocator2016::handleInitialState()
{
...
void SelfLocator2016::addHypothesesOnManualPositioningPositions()
{
  ...
  if (isGoalKeeper)
  {
    poseHypotheses.push_back(new PoseHypothesis2016(Pose2f(0, positionsByRules.goaliePosition),
  ...
  else
  {
    ...
    {
      poseHypotheses.push_back(new PoseHypothesis2016(Pose2f(0, positionsByRules.penaltyShootOutPosition),
    ...
    else
    {
      ...
      {
        poseHypotheses.push_back(new PoseHypothesis2016(Pose2f(0, position),
...
void SelfLocator2016::addHypothesesOnInitialKickoffPositions(float newPositionConfidence)
{
  poseHypotheses.push_back(new PoseHypothesis2016(Pose2f(-pi_2, theFieldDimensions.xPosOwnGroundline / 4.f, theFieldDimensions.yPosLeftSideline), newPositionConfidence, parameters.symmetryConfidence_whenPositionedManually, theFrameInfo.time, parameters));
  poseHypotheses.push_back(new PoseHypothesis2016(Pose2f(pi_2, theFieldDimensions.xPosOwnGroundline / 4.f, theFieldDimensions.yPosRightSideline), newPositionConfidence, parameters.symmetryConfidence_whenPositionedManually, theFrameInfo.time, parameters));
  poseHypotheses.push_back(new PoseHypothesis2016(Pose2f(-pi_2, theFieldDimensions.xPosOwnGroundline / 2.f, theFieldDimensions.yPosLeftSideline), newPositionConfidence, parameters.symmetryConfidence_whenPositionedManually, theFrameInfo.time, parameters));
  poseHypotheses.push_back(new PoseHypothesis2016(Pose2f(pi_2, theFieldDimensions.xPosOwnGroundline / 2.f, theFieldDimensions.yPosRightSideline), newPositionConfidence, parameters.symmetryConfidence_whenPositionedManually, theFrameInfo.time, parameters));
  poseHypotheses.push_back(new PoseHypothesis2016(Pose2f(-pi_2, 3 * theFieldDimensions.xPosOwnGroundline / 4.f, theFieldDimensions.yPosLeftSideline), newPositionConfidence, parameters.symmetryConfidence_whenPositionedManually, theFrameInfo.time, parameters));
  poseHypotheses.push_back(new PoseHypothesis2016(Pose2f(pi_2, 3 * theFieldDimensions.xPosOwnGroundline / 4.f, theFieldDimensions.yPosRightSideline), newPositionConfidence, parameters.symmetryConfidence_whenPositionedManually, theFrameInfo.time, parameters));
}

void SelfLocator2016::addHypothesesOnPenaltyPositions(float newPositionConfidence)
{
  for (auto &position : positionsByRules.penaltyPositions)
  {
    poseHypotheses.push_back(new PoseHypothesis2016(Pose2f(pi_2, position.x(), theFieldDimensions.yPosRightSideline), newPositionConfidence,
      parameters.symmetryConfidence_whenPositionedManually,
      theFrameInfo.time,
      parameters));
    poseHypotheses.push_back(new PoseHypothesis2016(Pose2f(-pi_2, position.x(), theFieldDimensions.yPosLeftSideline), newPositionConfidence,
      parameters.symmetryConfidence_whenPositionedManually,
      theFrameInfo.time,
      parameters));
  }
...
bool SelfLocator2016::addNewHypothesesWhenSymmetryLost()
{
...
bool SelfLocator2016::addNewHypothesesFromLineMatches()
{
  const bool isLost = localizationState == positionLost;

  const PoseHypothesis2016 &bestHypo = getBestHypothesis();
  Pose2f hypoPose;
...
bool SelfLocator2016::addNewHypothesesFromPenaltyCrossLine()
{
...
bool SelfLocator2016::addNewHypothesesFromCenterCirleAndLine()
{
...
bool SelfLocator2016::addNewHypothesesFromGoal()
{
  const bool isGoalKeeper = (theBehaviorData.role == BehaviorData::keeper);
  if (isGoalKeeper) return false; // Do not use for goalie
...
bool SelfLocator2016::addNewHypothesesFromLandmark()
{
...
bool SelfLocator2016::addNewHypotheses()
{
  bool added = false;
  ...
  case positionLost:
    ...
    if (parameters.landmarkBasedHypothesesSpawn == SelfLocator2016Parameters::spawnIfPositionLost
      || parameters.landmarkBasedHypothesesSpawn == SelfLocator2016Parameters::spawnIfLostOrTracking)
  ...
  case positionTracking:
    ...
    {
      added |= addNewHypothesesFromLineMatches();
      if (parameters.landmarkBasedHypothesesSpawn == SelfLocator2016Parameters::spawnIfPositionTracking
        || parameters.landmarkBasedHypothesesSpawn == SelfLocator2016Parameters::spawnIfLostOrTracking)
        added |= addNewHypothesesFromLandmark();
    }
    break;
...
float SelfLocator2016::getRobotPoseValidity(const PoseHypothesis2016 & poseHypothesis)
{
  float validity = 0;
...
inline void SelfLocator2016::initDebugging()
{
  DECLARE_DEBUG_DRAWING("module:SelfLocator2016:hypothesis", "drawingOnField");
  DECLARE_DEBUG_DRAWING("module:SelfLocator2016:correspondences", "drawingOnField");
  DECLARE_DEBUG_DRAWING("module:SelfLocator2016:LocalizationState", "drawingOnField");
  DECLARE_DEBUG_DRAWING("module:SelfLocator2016:poseFromCenterCircle", "drawingOnField");
  DECLARE_DEBUG_DRAWING("module:SelfLocator2016:poseFromPenaltyCross", "drawingOnField");

  MODIFY("module:SelfLocator2016:parameters", parameters);
}

void SelfLocator2016::doGlobalDebugging()
{
  ; // IMPORTANT!!! Do not remove!

  if (poseHypotheses.empty())
  {
    DRAWTEXT("module:SelfLocator2016:LocalizationState", -500, -2200, 50, ColorRGBA(255, 255, 255), "LocalizationState: no more hypotheses! Should never happen!");
    return;
  }
  ...
  COMPLEX_DRAWING("module:SelfLocator2016:hypothesis")
  {
    for (auto hypothesis : poseHypotheses)
    {
  ...
  {
  case positionTracking:
    DRAWTEXT("module:SelfLocator2016:LocalizationState", -500, -2200, 50, ColorRGBA(255, 255, 255), "LocalizationState: positionTracking");
    break;
  case positionLost:
    DRAWTEXT("module:SelfLocator2016:LocalizationState", -500, -2200, 50, ColorRGBA(255, 255, 255), "LocalizationState: positionLost");
    break;
  case fallenDown:
    DRAWTEXT("module:SelfLocator2016:LocalizationState", -500, -2200, 50, ColorRGBA(255, 255, 255), "LocalizationState: fallenDown");
    break;
  case penalized:
    DRAWTEXT("module:SelfLocator2016:LocalizationState", -500, -2200, 50, ColorRGBA(255, 255, 255), "LocalizationState: penalized");
    break;
  default:
    DRAWTEXT("module:SelfLocator2016:LocalizationState", -500, -2200, 50, ColorRGBA(255, 255, 255), "LocalizationState: default... should never happen!");
    // should never happen!
    break;
  }
}
...
const PoseHypothesis2016 &SelfLocator2016::getBestHypothesis()
{
  PoseHypotheses2016::iterator it;
...
void SelfLocator2016::addPoseToHypothesisVector(const Pose2f &pose, std::vector<PoseHypothesis2016*> &additionalHypotheses,
  const float &poseConfidence)
{
  ...
  Pose2f hypoPose;
  bestHypo.getRobotPose(hypoPose);
  float distancePose = (pose - hypoPose).translation.norm();
  float distanceSymPose = (symmetricPose - hypoPose).translation.norm();
  Pose2f nearestPose = (distancePose <= distanceSymPose) ? Pose2f(pose) : symmetricPose;
  ...
  if (posePossible && (isLost || allowToSpawn))
  {
    ...
    if (pc >= parameters.localizationState_positionLostWhenBestConfidenceBelowThisThreshold)
      additionalHypotheses.push_back(new PoseHypothesis2016(nearestPose,
      pc, bestHypo.getSymmetryConfidence(), theFrameInfo.time, parameters));
  }
}


MAKE_MODULE(SelfLocator2016, modeling)
