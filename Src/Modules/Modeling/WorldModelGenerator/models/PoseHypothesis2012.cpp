...
tls_type Eigen::Matrix<double, 2, PoseHypothesis2012::totalDimension>* PoseHypothesis2012::angleObservationMeasurementModelJacobian_H = 0;
tls_type Eigen::Matrix<double, 3, PoseHypothesis2012::totalDimension>* PoseHypothesis2012::infiniteLineMeasurementModelJacobian_H = 0;

tls_type PoseHypothesis2012::KalmanStateUpdateNew* PoseHypothesis2012::stateUpdateNew = 0;
tls_type PoseHypothesis2012::KalmanInfiniteLineStateUpdateNew* PoseHypothesis2012::stateUpdateInfiniteLineNew = 0;

PoseHypothesis2012::PoseHypothesis2012(const unsigned &timeStamp) :
positionConfidence(0.1f),
...
PoseHypothesis2012::PoseHypothesis2012(const Pose2f & newPose, float _positionConfidence, double _symmetryConfidence, const unsigned &timeStamp, const SelfLocator2012Parameters& parameters) :
...
PoseHypothesis2012::PoseHypothesis2012(const PoseHypothesis2012& other, const unsigned &timeStamp)
{
  *this = other;
  creationTime = timeStamp;
}

PoseHypothesis2012& PoseHypothesis2012::operator=(const PoseHypothesis2012& other)
{
  _uniqueId = getNextUniqueId();
  if (this == &other)
  {
...
void PoseHypothesis2012::cleanup()
{
  ...
  if (stateUpdateNew != 0)
  {
    delete stateUpdateNew;
    stateUpdateNew = 0;
  }
  if (stateUpdateInfiniteLineNew != 0)
  {
    delete stateUpdateInfiniteLineNew;
    stateUpdateInfiniteLineNew = 0;
  }
}

void PoseHypothesis2012::init(const Pose2f& newPose, const SelfLocator2012Parameters& parameters)
{
  initialized = true;
  ...
  if (stateUpdateNew == 0)
    stateUpdateNew = new PoseHypothesis2012::KalmanStateUpdateNew();
  stateUpdateNew->initMeasurementCovariance(singleMeasurementCovariance, parameters.sensorUpdate_correlationFactorBetweenMeasurements);

  if (stateUpdateInfiniteLineNew == 0)
    stateUpdateInfiniteLineNew = new PoseHypothesis2012::KalmanInfiniteLineStateUpdateNew();
  stateUpdateInfiniteLineNew->initMeasurementCovarianceInfiniteLine(singleInfiniteLineMeasurementCovariance, parameters.sensorUpdate_correlationFactorBetweenMeasurements);
...
void PoseHypothesis2012::predict(const Pose2f & odometryDelta, const SelfLocator2012Parameters& parameters)
{
  Pose2f tempPose;
  ...
  // TODO: find crash source?
  //ASSERT(std::abs(state[0]) < 100000);
...
void PoseHypothesis2012::fillCorrectionMatrices(
  ...
  const SelfLocator2012Parameters &parameters)
{
  Pose2f  robotPose;
  ...
  std::vector<Vector2d>& correspondencesInGlobalCoordsAngles = correspondencesInGlobalCoordsWithLocalFeaturePerceptionsSpherical;
    ...
    // don't use this, too much trouble with periodicities and singularities etc.
  {
    ...
    if (calculateMeasurementLikelihoodSpherical(measurement.measurement, realMeasurement.observation, parameters) > 0)
    {
      ...
      correspondencesInGlobalCoordsAngles.push_back(Vector2d(0, 0));

      Vector2f obsInFieldCoords = robotPose * Vector2f(relativePosition.cast<float>());
      LINE("module:SelfLocator2012:correspondences",
      ...
      ARROW("module:SelfLocator2012:correspondences",
    ...
    // too much trouble with periodicities and singularities etc.
  {
    ...
    if (!useOwnMark && calculateMeasurementLikelihoodSpherical(expectedMeasurementOpp, realMeasurement.observation, parameters) > 0)
    {
      ...
      observationsAsAngles.push_back(realMeasurement);
      correspondencesInGlobalCoordsAngles.push_back(penaltyMarkOpp);
    ...
    else if (useOwnMark && calculateMeasurementLikelihoodSpherical(expectedMeasurementOwn, realMeasurement.observation, parameters) > 0)
    {
      ...
      observationsAsAngles.push_back(realMeasurement);
      correspondencesInGlobalCoordsAngles.push_back(penaltyMarkOwn);
    ...
    && theGoalPercept.numberOfGoalPosts < 3) // one of them is a false positive anyway and using any of them is risky
  {
    for (auto goalPost : theGoalPercept.goalPosts)
    {
      ...
      Vector2d bestCorrespondenceInGlobalCoords;
      if (findGoalPostMatch(goalPost, theCameraMatrix, theCameraMatrixUpper, realMeasurement, bestCorrespondenceInGlobalCoords, theFieldDimensions, parameters))
      {
        ...
        ARROW("module:SelfLocator2012:correspondences",
          obsInFieldCoords.x(), obsInFieldCoords.y(), bestCorrespondenceInGlobalCoords.x(), bestCorrespondenceInGlobalCoords.y(), 30, Drawings::solidPen, ColorRGBA(200, 200, 0));
      }
    }
  }
...
void PoseHypothesis2012::updateStateRotationWithLocalFieldLines(
...
bool PoseHypothesis2012::updatePositionConfidenceWithLocalFeaturePerceptionsWeighted(
  const CLIPFieldLinesPercept &theFieldLinesPercept,
  const CLIPCenterCirclePercept &theCenterCirclePercept,
  const CLIPGoalPercept &theGoalPercept,
  const PenaltyCrossPercept &thePenaltyCrossPercept,
...
// LEVEL 1
bool PoseHypothesis2012::updatePositionConfidenceWithSingleLines(
  const CLIPFieldLinesPercept & theFieldLinesPercept,
  const FieldDimensions & theFieldDimensions,
  const SelfLocator2012Parameters & parameters,
...
// LEVEL 2
bool PoseHypothesis2012::updatePositionConfidenceWithLineCrossings(
  const CLIPFieldLinesPercept & theFieldLinesPercept,
  const FieldDimensions & theFieldDimensions,
  const SelfLocator2012Parameters & parameters,
  const Pose2f &robotPose,
...
// LEVEL 3
bool PoseHypothesis2012::updatePositionConfidenceWithLineAndLandmark(
  const CLIPFieldLinesPercept & theFieldLinesPercept,
  const CLIPCenterCirclePercept &theCenterCirclePercept,
  const CLIPGoalPercept &theGoalPercept,
  const PenaltyCrossPercept &thePenaltyCrossPercept,
  ...
  const std::vector<int> &linesVertical)
{
  ...
  if (PoseGenerator::getPoseFromCenterCircleAndCenterLine(theFieldLinesPercept, theCenterCirclePercept, pose, perceptWeight) > 0)
  {
    ...
    DRAW_ROBOT_POSE("module:SelfLocator2012:poseFromCenterCircle", closestPose, ColorRGBA(255 - (unsigned char)(perceptWeight * 255.f), (unsigned char)(perceptWeight * 255.f), 0));
  }
  ...
  if (PoseGenerator::getPoseFromPenaltyCrossAndLine(theFieldDimensions, theFieldLinesPercept, thePenaltyCrossPercept, pose, perceptWeight))
  {
    ...
    DRAW_ROBOT_POSE("module:SelfLocator2012:poseFromPenaltyCross", closestPose, ColorRGBA(0, 0, 255));
  }
  return update;
}

bool PoseHypothesis2012::updatePositionConfidenceWithLocalFeaturePerceptionsSpherical(
  const LineMatchingResult &theLineMatchingResult,
  const CLIPCenterCirclePercept &theCenterCirclePercept,
  const CLIPGoalPercept &theGoalPercept,
  const PenaltyCrossPercept &thePenaltyCrossPercept,
  ...
  const SelfLocator2012Parameters & parameters)
{
  ...
  MODIFY("module:SelfLocator2012:requestLineCorrespondencesForThisPose", requestCorrespondencesForThisPose);
  MODIFY("module:SelfLocator2012:lineCorrespondenceLikelihoodThreshold", likelihoodThreshold);
...
void PoseHypothesis2012::updateStateWithLocalFeaturePerceptionsSpherical(
  const LineMatchingResult &theLineMatchingResult,
  const CLIPCenterCirclePercept &theCenterCirclePercept,
  const CLIPGoalPercept &theGoalPercept,
  const PenaltyCrossPercept &thePenaltyCrossPercept,
...
{
  // TODO: find crash source? 
  //ASSERT(fabs(state[0]) < 100000);
  //will be filled in updatePositionConfidence
  std::vector<AngleObservation>& observationsAsAngles = observationAnglesWithLocalFeaturePerceptionsSpherical;
  std::vector<Vector2d>& correspondencesInGlobalCoords = correspondencesInGlobalCoordsWithLocalFeaturePerceptionsSpherical;
  ...
  {
    OUTPUT_TEXT("PoseHypothesis2012::updateWithLocalFeaturePerceptions(...): Consider increasing maxMeasurementDimension!");
  }
  ...
  std::vector<ExpectedMeasurementLandmark<totalDimension> > measurements;
  for (size_t landmarkIndex = 0; landmarkIndex < correspondencesInGlobalCoords.size(); landmarkIndex++)
    measurements.push_back(
    calculateMeasurementModelForAngularLandmarkObservation(correspondencesInGlobalCoords[landmarkIndex], observationsAsAngles[landmarkIndex].cameraHeight, parameters));

  if (stateUpdateNew == 0)
    stateUpdateNew = new PoseHypothesis2012::KalmanStateUpdateNew();

  Eigen::Matrix<double, totalDimension, 1> correction = stateUpdateNew->updateWithLocalObservations(observationsAsAngles, measurements, covariance);
  if (correction.isMuchSmallerThan(0.1))
    OUTPUT_TEXT("Minimal state correction neglected");
  else
    state += correction;

  // TODO: find crash source? always crashes here, not at start of method!!!
  //ASSERT(std::abs(state[0]) < 100000);
  observationsAsAngles.clear(); //clear so we (k)now whats up
  correspondencesInGlobalCoords.clear();
}

bool PoseHypothesis2012::updatePositionConfidenceWithLocalFeaturePerceptionsInfiniteLines(
  const LineMatchingResult & theLineMatchingResult,
  const CLIPCenterCirclePercept &theCenterCirclePercept,
  const FieldDimensions & theFieldDimensions,
  const SelfLocator2012Parameters & parameters)
{
  ...
  if (theLineMatchingResult.containsNonUniqueMatches())
  {
    std::vector<LineMatchingResult::FieldLine> correspondencesForObservations;
    foundCorrespondenceInLineMatch = theLineMatchingResult.getCorrespondencesForLocalizationHypothesis(robotPose, poseCovarianceForLineMatching, likelihoodThreshold, singleMeasurementCovariance_2x2, correspondencesForObservations);
    // do not update the confidence, as this is such a weak match compared to the others...
    // Well, in the negative case this is still a useful indication, so update the confidence then.
    if (!foundCorrespondenceInLineMatch)
    {
      updatePositionConfidence(false, parameters.sensorUpdate_influenceOfNewInfiniteLineMeasurementOnPositionConfidence, 1.f);
    }
    update = foundCorrespondenceInLineMatch;
  }
  return update;
}
void PoseHypothesis2012::updateStateWithLocalFeaturePerceptionsInfiniteLines(
  const LineMatchingResult & theLineMatchingResult,
  const SelfLocator2012Parameters & parameters)
{
  std::vector<NormalObservation>& observationsAsNormals = observationsAsNormalsWithLocalFeaturePerceptionsInfiniteLines;
  ...
  int dimensionOfMeasurementVector = static_cast<int>(observationsAsNormals.size() * 3); // each observation has 2 angles

  // ok, since we only have limited sized matrizes for the update, we restrict the measurement dimension...
  const int maxMeasurementDimension = 3 * MAX_OBSERVATIONS_INFINITELINE;
  if (parameters.displayWarnings && dimensionOfMeasurementVector > maxMeasurementDimension)
  {
    OUTPUT_TEXT("PoseHypothesis2012::updateStateWithLocalFeaturePerceptionsInfiniteLines(...): Consider increasing maxMeasurementDimension!");
  }
  while (dimensionOfMeasurementVector > maxMeasurementDimension)
  {
  ...
  for (size_t landmarkIndex = 0; landmarkIndex < observationsAsNormals.size(); landmarkIndex++)
    ...
    correspondencesInGlobalCoords[landmarkIndex * 2 + 1], observationsAsNormals[landmarkIndex].cameraHeight, parameters));

  if (stateUpdateInfiniteLineNew == 0)
    stateUpdateInfiniteLineNew = new PoseHypothesis2012::KalmanInfiniteLineStateUpdateNew();

  Eigen::Matrix<double, totalDimension, 1> correction = stateUpdateInfiniteLineNew->updateWithLocalInfiniteLineObservations(observationsAsNormals, measurements, covariance);
  state += correction;
}

void PoseHypothesis2012::updateWithLocalFeaturePerceptionsCartesian(
  const CLIPCenterCirclePercept &theCenterCirclePercept,
...
bool PoseHypothesis2012::findGoalPostMatch(
  ...
  const SelfLocator2012Parameters& parameters)
{
  ...
  if (goalPost.goalPostSide != CLIPGoalPercept::GoalPost::rightPost) // unknown or left
  {
    ...
    expectedMeasurement.x() = measurement.measurement.x();
    expectedMeasurement.y() = measurement.measurement.y();
    likelihood = calculateMeasurementLikelihoodSpherical(realMeasurement.observation, expectedMeasurement, parameters);
    if (likelihood > bestLikelihood)
    {
      bestLikelihood = likelihood;
      bestCorrespondenceInGlobalCoords = correspondenceInGlobalCoords;
    }
...
ExpectedMeasurementLandmark<PoseHypothesis2012::totalDimension> PoseHypothesis2012::calculateMeasurementModelForAngularLandmarkObservation(
  const Vector2d & correspondenceInGlobalCoords, const float &cameraHeight, const SelfLocator2012Parameters & parameters)
{
  if (angleObservationMeasurementModelJacobian_H == 0)
    angleObservationMeasurementModelJacobian_H = new Eigen::Matrix<double, 2, PoseHypothesis2012::totalDimension>();
  *angleObservationMeasurementModelJacobian_H = Eigen::Matrix<double, 2, PoseHypothesis2012::totalDimension>::Zero();
  ...
  expectedAngularMeasurement.weight = 1. - ((expectedAngularMeasurement.measurement.x() - fullWeightAngle) / (pi_2 - fullWeightAngle));
...
ExpectedMeasurementInfiniteLine<PoseHypothesis2012::totalDimension> PoseHypothesis2012::calculateMeasurementModelForInfiniteLineObservation(const Vector2d & startPointCorrespondenceInGlobalCoords,
  const Vector2d & endPointCorrespondenceInGlobalCoords, const float &cameraHeight, const SelfLocator2012Parameters & parameters)
{
  if (infiniteLineMeasurementModelJacobian_H == 0)
    infiniteLineMeasurementModelJacobian_H = new Eigen::Matrix<double, 3, PoseHypothesis2012::totalDimension>();
  *infiniteLineMeasurementModelJacobian_H = Eigen::Matrix<double, 3, PoseHypothesis2012::totalDimension>::Zero();
...
void PoseHypothesis2012::updateSymmetryByComparingRemoteToLocalModels(
  ...
  const SelfLocator2012Parameters & parameters)
{
  ...
  // check ball
  if (frameInfo.getTimeSince(theBallModel.timeWhenLastSeen) < 500
    && frameInfo.getTimeSince(theRemoteBallModel.timeWhenLastSeen) < 1000
    && theBallModel.validity > 0.7f && theRemoteBallModel.validity > 0.8f) // TODO validity threshold to parameters!!
  {
    ...
    {
      updateSymmetryConfidence(difference < differenceOfMirrored, parameters);
    }
  }
...
void PoseHypothesis2012::draw(ColorRGBA myselfColor) const
{
  ...
  POSE_2D_SAMPLE("module:SelfLocator2012:hypothesis", pose, myselfColor);

  DRAWTEXT("module:SelfLocator2012:hypothesis", pose.translation.x(), pose.translation.y() - 180, 50, myselfColor, "pc: " << positionConfidence);
  DRAWTEXT("module:SelfLocator2012:hypothesis", pose.translation.x(), pose.translation.y() - 300, 50, myselfColor, "sc: " << symmetryConfidence);
}

void PoseHypothesis2012::drawGaussian(const GaussianDistribution2D &gd, const ColorRGBA &color, int penSize, bool drawCenter) const
{
