...
#include "Modules/Modeling/WorldModelGenerator/SelfLocator2012Parameters.h"
#include "PoseHypothesis2012_Defines.h"
...
public:
  ...
  PoseHypothesis2012(const unsigned &timeStamp);

  PoseHypothesis2012(const Pose2f & newPose, float _positionConfidence, double _symmetryConfidence, const unsigned &timeStamp, const SelfLocator2012Parameters& parameters);
  PoseHypothesis2012(const PoseHypothesis2012& other, const unsigned &timeStamp);
  PoseHypothesis2012& operator=(const PoseHypothesis2012& other);

private:
  typedef KalmanStateUpdateObservationsNew<totalDimension, 2 * MAX_OBSERVATIONS> KalmanStateUpdateNew;
  typedef KalmanStateUpdateInfiniteLinesNew<totalDimension, 3 * MAX_OBSERVATIONS_INFINITELINE> KalmanInfiniteLineStateUpdateNew;
  ...
  std::vector<Vector2d> correspondencesInGlobalCoordsWithLocalFeaturePerceptionsSpherical;
  ...
  tls KalmanStateUpdateNew* stateUpdateNew;
  tls KalmanInfiniteLineStateUpdateNew* stateUpdateInfiniteLineNew;
...
public:
  ...
  void init(const Pose2f & newPose, const SelfLocator2012Parameters& parameters);

  void predict(const Pose2f & odometryDelta, const SelfLocator2012Parameters& parameters);
  ...
  // this is the normal sensor update used for all features (but for the exceptions below)
  void updateStateWithLocalFeaturePerceptionsSpherical(
    const LineMatchingResult & theLineMatchingResult,
    const CLIPCenterCirclePercept &theCenterCirclePercept,
    const CLIPGoalPercept &theGoalPercept,
    const PenaltyCrossPercept &thePenaltyCrossPercept,
  ...
  bool updatePositionConfidenceWithLocalFeaturePerceptionsInfiniteLines(
    ...
    const FieldDimensions & theFieldDimensions,
    const SelfLocator2012Parameters & parameters);
  void updateStateWithLocalFeaturePerceptionsInfiniteLines(
    const LineMatchingResult & theLineMatchingResult,
    const SelfLocator2012Parameters & parameters);
...
private:
  ...
  ExpectedMeasurementInfiniteLine<totalDimension> calculateMeasurementModelForInfiniteLineObservation(
    const Vector2d & startPointCorrespondenceInGlobalCoords, const Vector2d & endPointCorrespondenceInGlobalCoords, const float &cameraHeight, const SelfLocator2012Parameters & parameters);
  ...
  void updateSymmetryConfidence(bool thisPositionMoreLikeliThanMirroredOne, const SelfLocator2012Parameters& parameters)
  {
    symmetryConfidence *= (1.0 - parameters.sensorUpdate_influenceOfNewMeasurementOnSymmetryConfidence);
    symmetryConfidence += thisPositionMoreLikeliThanMirroredOne ? parameters.sensorUpdate_influenceOfNewMeasurementOnSymmetryConfidence : 0;
    symmetryConfidence = std::max(symmetryConfidence, 0.01);
  }

  double calculateMeasurementLikelihoodSpherical(const Vector2d & angles1, const Vector2d & angles2, const SelfLocator2012Parameters& parameters) const
  {
  ...
  inline float calcWeightForPoseDifference(const Pose2f &poseDiff, const SelfLocator2012Parameters &parameters)
  {
