/**
* @file PoseHypothesis2012_Defines.h
*
...
class KalmanStateUpdateObservations
{
private:
...
public:
    ...
    Eigen::Matrix<double, stateDim, stateDim> & covariance)
  {
    ...
    const int n = nDim;
    measurementModelJacobian_H.clear();
    for (int row = 0; row < n; row += 2)
    {
      ...
      measurementModelJacobian_H.block(row, 0, 1, stateDim) = expectedAngles[landmarkIndex].measurementModelJacobian->block(0, 0, 1, stateDim);
    }
    measurementModelJacobian_H_transposed = measurementModelJacobian_H.transpose();
    //tempVector = tempVector - expectedMeasurement;
    innovationCovariance = measurementModelJacobian_H * covariance * measurementModelJacobian_H_transposed;
    innovationCovariance.addToThisMatrix(measurementCovariance);
    kalmanGain = covariance * (measurementModelJacobian_H_transposed * innovationCovariance.inverse());
    movement = kalmanGain * tempVector;
    measurementModelJacobian_H *= -1.0;
    tempMatrixOfFullDimension = kalmanGain * measurementModelJacobian_H;
...
template<int stateDim, int nDim>
class KalmanStateUpdateObservationsNew
{
private:
  Eigen::Matrix<double, nDim, stateDim> measurementModelJacobian_H;
  Eigen::Matrix<double, stateDim, nDim> measurementModelJacobian_H_transposed;
...
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  KalmanStateUpdateObservationsNew() :
    measurementModelJacobian_H(Eigen::Matrix<double, nDim, stateDim>::Zero()),
    measurementModelJacobian_H_transposed(Eigen::Matrix<double, stateDim, nDim>::Zero()),
    innovationCovariance(Eigen::Matrix<double, nDim, nDim>::Zero()),
    measurementCovariance(Eigen::Matrix<double, nDim, nDim>::Zero()),
    ...
    Eigen::Matrix<double, stateDim, stateDim> & covariance)
  {
    ...
    measurementModelJacobian_H.block(0,0,n,stateDim) = Eigen::Matrix<double, nDim, stateDim>::Zero().block(0,0,n,stateDim);
    for (int row = 0; row < n; row += 2)
    {
      const int landmarkIndex = row / 2;
      expectedMeasurement[row] = expectedAngles[landmarkIndex].measurement.x();
    ...
    //tempVector = tempVector - expectedMeasurement;
    innovationCovariance.block(0,0,n,n) = measurementModelJacobian_H.block(0,0,n,stateDim)*covariance.block(0,0,stateDim,stateDim)*measurementModelJacobian_H_transposed.block(0,0,stateDim,n);
    innovationCovariance.block(0,0,n,n) += measurementCovariance.block(0,0,n,n);
    kalmanGain.block(0,0,stateDim,n) = covariance.block(0,0,stateDim,stateDim)*(measurementModelJacobian_H_transposed.block(0,0,stateDim,n)*innovationCovariance.block(0,0,n,n).inverse());
...
template<int stateDim, int nDim>
class KalmanStateUpdateInfiniteLines
{
private:
...
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  KalmanStateUpdateInfiniteLines() :
    ...
    Eigen::Matrix<double, stateDim, stateDim> & covariance)
  {
    ...
    for (int row = 0; row<n; row += stateDim)
    {
      ...
      infiniteLineMeasurementModelJacobian_H.block(row, 0, 2, stateDim) = expectedMeasurements[landmarkIndex].measurementModelJacobian->block(0, 0, 2, stateDim);
    }
    infiniteLineMeasurementModelJacobian_H_transposed = infiniteLineMeasurementModelJacobian_H.transpose();
    //infiniteLineTempVector = infiniteLineTempVector - infiniteLineExpectedMeasurement;
    infiniteLineInnovationCovariance = infiniteLineMeasurementModelJacobian_H * covariance * infiniteLineMeasurementModelJacobian_H_transposed;
    infiniteLineInnovationCovariance += infiniteLineMeasurementCovariance;
    infiniteLineKalmanGain = covariance * (infiniteLineMeasurementModelJacobian_H_transposed * infiniteLineInnovationCovariance.inverse());
    movement = infiniteLineKalmanGain * infiniteLineTempVector;
    infiniteLineMeasurementModelJacobian_H *= -1.0;
    tempMatrixOfFullDimension = infiniteLineKalmanGain * infiniteLineMeasurementModelJacobian_H;
    for (int i = 0; i < stateDim; i++)
    {
      tempMatrixOfFullDimension(i, i) += 1.0;
    }
...
template<int stateDim, int nDim>
class KalmanStateUpdateInfiniteLinesNew
{
private:
  Eigen::Matrix<double, nDim, stateDim> infiniteLineMeasurementModelJacobian_H;
  Eigen::Matrix<double, stateDim, nDim> infiniteLineMeasurementModelJacobian_H_transposed;
...
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  KalmanStateUpdateInfiniteLinesNew() :
    infiniteLineMeasurementModelJacobian_H(Eigen::Matrix<double, nDim, stateDim>::Zero()),
    infiniteLineMeasurementModelJacobian_H_transposed(Eigen::Matrix<double, stateDim, nDim>::Zero()),
    infiniteLineInnovationCovariance(Eigen::Matrix<double, nDim, nDim>::Zero()),
    infiniteLineMeasurementCovariance(Eigen::Matrix<double, nDim, nDim>::Zero()),
    ...
    Eigen::Matrix<double, stateDim, stateDim> & covariance)
  {
    ...
    //infiniteLineTempVector = infiniteLineTempVector - infiniteLineExpectedMeasurement;
    infiniteLineInnovationCovariance.block(0,0,n,n) = infiniteLineMeasurementModelJacobian_H.block(0,0,n,stateDim) * covariance.block(0,0,stateDim,stateDim) * infiniteLineMeasurementModelJacobian_H_transposed.block(0,0,stateDim,n);
    infiniteLineInnovationCovariance.block(0,0,n,n) += infiniteLineMeasurementCovariance.block(0,0,n,n);
    infiniteLineKalmanGain.block(0,0,stateDim,n) = covariance.block(0,0,stateDim,stateDim)*(infiniteLineMeasurementModelJacobian_H_transposed.block(0,0,stateDim,n)*infiniteLineInnovationCovariance.block(0, 0, n, n).inverse());
    ...
    tempMatrixOfFullDimension.block(0,0,stateDim, stateDim) = infiniteLineKalmanGain.block(0,0,stateDim, n) * infiniteLineMeasurementModelJacobian_H.block(0,0,n, stateDim);
    for (int i = 0; i < stateDim; i++)
    {
      tempMatrixOfFullDimension(i,i) += 1.0;
    }
