...
public:
  ...
  SphericalObservation()
    : realAngles(Vector2d::Zero()), nominalAngles(Vector2d::Zero()),
      measurementModelJacobian(JacobianMatrix::Zero()), weight(0) {}
};

template<int dim>
class InfiniteLineObservation
{
public:
  ...
  InfiniteLineObservation()
    : realNormals(Vector3d::Zero()), nominalNormals(Vector3d::Zero()),
      measurementModelJacobian(JacobianMatrix::Zero()), weight(0) {}
};

template<int dim>
using SphericalObservationVector = std::vector<SphericalObservation<dim>, Eigen::aligned_allocator<SphericalObservation<dim> > >;
template<int dim>
using InfiniteLineObservationVector = std::vector<InfiniteLineObservation<dim>, Eigen::aligned_allocator<InfiniteLineObservation<dim> > >;

template<int stateDim, int nDim>
class KalmanStateUpdateObservations2017
{
private:
...
public:
  ...
  Eigen::Matrix<double, stateDim, 1> updateWithLocalObservations(
    const SphericalObservationVector<stateDim>& sphericalObservations,
...
template<int stateDim, int nDim>
class KalmanStateUpdateInfiniteLines2017
{
private:
...
public:
  ...
  Eigen::Matrix<double, stateDim, 1> updateWithLocalInfiniteLineObservations(
    const InfiniteLineObservationVector<stateDim> & infiniteLineObservations,
    Eigen::Matrix<double, stateDim, stateDim> & covariance)
  {
