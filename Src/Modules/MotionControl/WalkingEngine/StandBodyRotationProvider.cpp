...
void StandBodyRotationProvider::update(StandBodyRotation& standBodyRotation)
{
  ...
  if(theMotionSelection.ratios[MotionRequest::walk] != 1.f)
  {
    if(standBodyRotation.bodyRotation.x() < 0.f)
      standBodyRotation.bodyRotation = Vector2f(std::max(standBodyRotation.bodyRotation.x(), 0.5f * -maxXRotation * theMotionSelection.ratios[MotionRequest::walk]), 0.f);
    else
      standBodyRotation.bodyRotation = Vector2f(std::min(standBodyRotation.bodyRotation.x(), 0.5f * maxXRotation * theMotionSelection.ratios[MotionRequest::walk]), 0.f);
