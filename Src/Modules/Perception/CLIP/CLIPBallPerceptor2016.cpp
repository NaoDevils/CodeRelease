#include "CLIPBallPerceptor2016.h"
#include "Tools/Math/Geometry.h"
#include "Tools/Math/Random.h"
#include "Tools/RingBufferWithSum.h"

CLIPBallPerceptor2016::CLIPBallPerceptor2016()
{
...
void CLIPBallPerceptor2016::update(BallPercept &theBallPercept)
{
  DECLARE_DEBUG_DRAWING("module:CLIPBallPerceptor2016:additionalScan", "drawingOnImage");
  DECLARE_DEBUG_DRAWING("module:CLIPBallPerceptor2016:ballScanLines", "drawingOnImage");
  DECLARE_DEBUG_DRAWING("module:CLIPBallPerceptor2016:ballScannedCenter", "drawingOnImage");
  DECLARE_DEBUG_DRAWING("module:CLIPBallPerceptor2016:ballValidity", "drawingOnImage");
  DECLARE_DEBUG_DRAWING("module:CLIPBallPerceptor2016:yJumpScanLines", "drawingOnImage");
  DECLARE_DEBUG_DRAWING("module:CLIPBallPerceptor2016:ballFeatures", "drawingOnImage");
  DECLARE_DEBUG_DRAWING("module:CLIPBallPerceptor2016:fittingPoints", "drawingOnImage");
  DECLARE_DEBUG_DRAWING("module:CLIPBallPerceptor2016:testCircles", "drawingOnImage");
  //DECLARE_DEBUG_DRAWING("module:CLIPBallPerceptor2016:featureHistogram", "drawingOnImage");
  //DECLARE_DEBUG_DRAWING("module:CLIPBallPerceptor2016:distanceHistogram", "drawingOnImage");
...
void CLIPBallPerceptor2016::execute(const bool &upper)
{
...
bool CLIPBallPerceptor2016::verifyBallPercept(BallSpot &spot, const bool &upper)
{
  ...
  DRAWTEXT("module:CLIPBallPerceptor2016:ballValidity",
  ...
  {
    CIRCLE("module:CLIPBallPerceptor2016:ballScanLines",
...
bool CLIPBallPerceptor2016::checkYJumps(BallSpot &spot, const bool &upper)
{
  const Image &image = upper ? (Image&)theImageUpper : theImage;
  ...
  for (int i = 0; i < 16; i++)
  {
    ...
    yDir += yInc;
    yJumpSum += yJumpLine;
    LINE("module:CLIPBallPerceptor2016:yJumpScanLines",
...
bool CLIPBallPerceptor2016::checkFeatures(const BallSpot &spot, const bool &upper)
{
  const Image &image = upper ? (Image&)theImageUpper : theImage;
  ...
  for (int row = 0; row < 15; row++)
  {
    ...
    for (int xPos = minX; xPos < maxX; xPos++)
    {
      ...
      //LINE("module:CLIPBallPerceptor2016:ballFeatures", minX, yPos, maxX, yPos, 2, Drawings::solidPen, ColorRGBA(0, 255, 0));
    }
    lastYPos = yPos;
  }
  if (pixelCount == 0)
  ...
  //DRAWTEXT("module:CLIPBallPerceptor2016:ballFeatures", spot.position.x(), spot.position.y(), spot.radiusInImage / 2, ColorRGBA::magenta, "S:" << score << "," << whiteScore);
  ...
  for (auto &bf : ballPerceptState.ballFeatures)
  {
    CIRCLE("module:CLIPBallPerceptor2016:ballFeatures",
...
bool CLIPBallPerceptor2016::checkFeatureDistribution(const BallSpot &spot, const bool &upper)
{
  return true;
}

bool CLIPBallPerceptor2016::scanForOverlap(const BallSpot &spot, const bool &upper)
{
  int overlapCount = 0;
...
bool CLIPBallPerceptor2016::scanFeature(BallFeature &feature, const Vector2f &center, const float &expectedSize, const bool &upper)
{
  const Image &image = upper ? (Image&)theImageUpper : theImage;
  Vector2f dir(1.f, 0);
...
int CLIPBallPerceptor2016::getFittingPoints(const Geometry::Circle &baseCircle,
  float &distSum, float &maxDist, const bool &upper)
{
  ...
  {
    DOT("module:CLIPBallPerceptor2016:fittingPoints", point->pointInImage.x(), point->pointInImage.y(), ColorRGBA::yellow, ColorRGBA::yellow);
...
bool CLIPBallPerceptor2016::isOnBallSpot(const Vector2i &pos, const bool &upper)
{
...
void CLIPBallPerceptor2016::additionalBallSpotScan()
{
  ...
  {
    LINE("module:CLIPBallPerceptor2016:additionalScan", imgXStart, imgY, imgXEnd, imgY, 2, Drawings::solidPen, ColorRGBA::orange);
    runBallSpotScanLine(imgX, imgY, stepSize, 0, stepsX, upper);
  }

  for (imgX = imgXStart; imgX < imgXEnd; imgX += xStepSize)
  {
    LINE("module:CLIPBallPerceptor2016:additionalScan", imgX, imgYStart, imgX, imgYEnd, 2, Drawings::solidPen, ColorRGBA::orange);
    runBallSpotScanLine(imgX, imgYEnd, 0, stepSize, stepsY, upper);
  }

}

void CLIPBallPerceptor2016::runBallSpotScanLine(int x, int y, int stepX, int stepY, int steps, const bool &upper)
{
  const Image &image = upper ? (Image&)theImageUpper : theImage;
  const FieldColors &fieldColor = upper ? (FieldColors&)theFieldColorsUpper : theFieldColors;
  const CameraInfo &cameraInfo = upper ? (CameraInfo&)theCameraInfoUpper : theCameraInfo;
  ...
  while (stepNo < steps && !image.isOutOfImage(x, y, 10))
  {
    ...
    if (ballSegmentSize < lastBallSegmentSize)
    {
      if (lastBallSegmentSize*step > expectedBallSize / 2 && lastBallSegmentSize*step < expectedBallSize * 2)
      {
        ...
        LINE("module:CLIPBallPerceptor2016:additionalScan", x, y,
...
int CLIPBallPerceptor2016::countGreenOnBallSpot(BallSpot &spot, const bool &upper)
{
  const Image &image = upper ? (Image&)theImageUpper : (Image&)theImage;
  const FieldColors &fieldColor = upper ? (FieldColors&)theFieldColorsUpper : theFieldColors;
...
bool CLIPBallPerceptor2016::calcBallSpot2016(BallSpot &spot, const bool &upper)
{
  // TODO: shorten/split
  ...
  for (i = 0; i < maxI; i++)
  {
    ...
    for (int j = 0; scanLength < maxScanSize && !image.isOutOfImage(scanPoint.x(), scanPoint.y(), 4); j++)
    {
      ...
      else
      {
        ...
        if (scanLength < yJumpRadius)
        {
          ...
          {
            maxY = 0;
            /*DOT("module:CLIPBallPerceptor2016:ballFeatures",
    ...
    LINE("module:CLIPBallPerceptor2016:ballScanLines",
    spot.position.x, spot.position.y,
    scanPoint.x, scanPoint.y,
  ...
  DEBUG_DRAWING("module:CLIPBallPerceptor2016:overlapAreasRaw", "drawingOnImage")
  {
    if (spot.found)
    {
      ...
      for (auto &ova : ballPerceptState.overlapAreas)
      {
        ...
        points[2] += points[0];
        POLYGON("module:CLIPBallPerceptor2016:overlapAreasRaw", numOfPoints, points, 3, Drawings::solidPen, ColorRGBA::red, Drawings::solidBrush, ColorRGBA(0, 0, 0, 0));
      }
    }
  }
  ...
  CROSS("module:CLIPBallPerceptor2016:ballScannedCenter", scannedCenter.x(), scannedCenter.y(), 5, 2, Drawings::solidPen, centerColor);
  ...
  else // checks for 3rd call (ball hull verification)
  {
    ...
    else if (ballPerceptState.circleOK && theFieldDimensions.ballType == SimpleFieldDimensions::BallType::whiteBlack)
    {
      ...
      if (true)//(posOnField - theBallModel.estimate.position).abs() < 500)
      {
        ...
        COMPLEX_DRAWING("module:CLIPBallPerceptor2016:ballScanLines")
        {
...
bool CLIPBallPerceptor2016::verifyBallHull(BallSpot &spot, const bool &upper)
{
  ...
  if (Geometry::computeCircleOnFieldLevenbergMarquardt(goodBallPoints, testCircle) && testCircle.radius < imageHeight / 4)
  {
    ...
    if (yClipped - 2 > testCircle.center.y())
      return false;
    CIRCLE("module:CLIPBallPerceptor2016:testCircles",
    ...
    DRAWTEXT("module:CLIPBallPerceptor2016:fittingPoints", testCircle.center.x(), testCircle.center.y() - 5, 10, ColorRGBA::yellow, "" << fittingPoints);
...
MAKE_MODULE(CLIPBallPerceptor2016, perception)
