...
void CLIPGoalPerceptor2015::connectGoalSpots(const bool& upper)
{
  ...
  for (; spot != end; ++spot)
  {
    ...
    {
      if (testSpot->yPos - spot->yPos == scanLineDistance && abs(testSpot->xPos - spot->xPos) < scanLineDistance) //TODO: param
      {
        ...
        {
          LINE("module:CLIPGoalPerceptor:spotConnections", spot->xPos, spot->yPos, testSpot->xPos, testSpot->yPos, 2, Drawings::solidPen, ColorRGBA::black);
  ...
  for (; spot != end; ++spot)
  {
    ...
    if (noSpots > 2) // TODO: param
    {
      ...
      if (std::abs(lineAngle - avgAngle) < 0.2f)
      {
        ...
        LINE("module:CLIPGoalPerceptor:goalPostLinesFinal", testLine.base.x(), testLine.base.y(), testLine.base.x() + testLine.direction.x(), testLine.base.y() + testLine.direction.y(), 3, Drawings::solidPen, ColorRGBA::yellow);
      }
    }
  }
}
...
void CLIPGoalPerceptor2015::verifyGoalPosts(CLIPGoalPercept& theCLIPGoalPercept, const bool& upper)
{
  ...
  for (; gp != goalPosts.end();)
  {
    ...
    if ((*gp).validity > minValidity && (!useGoalColor || abs((*gp).avgY - expectedGoalY) < 60) && ((*gp).endInImage.y() - (*gp).startInImage.y()) > imageHeight / 12) // TODO: param
    {
      ...
      if (!newGPUseless)
      {
        ...
        else
        {
          ...
          if (!Transformation::robotToImage(Vector3f((float)newGP.locationOnField.x(), (float)newGP.locationOnField.y(), theFieldDimensions.goalHeight), cameraMatrix, cameraInfo, pImage))
          {
...
void CLIPGoalPerceptor2015::mergeGoalPostInfo(CLIPGoalPercept& theCLIPGoalPercept)
{
  ...
  while (gp != theCLIPGoalPercept.goalPosts.end())
  {
    ...
    while (gpOther != theCLIPGoalPercept.goalPosts.end())
    {
      ...
      if (std::abs(dist - 2 * theFieldDimensions.yPosLeftGoal) < maxDist && std::abs(gp->topWidth - gpOther->topWidth) < std::max<float>(gp->topWidth / 10, 3.f))
      {
...
bool CLIPGoalPerceptor2015::scanForGoalPostBottom(GoalPost& gp, const bool& upper)
{
  ...
  int length = scanSameColor(startPoint, direction, (int)(1 + (gp.topWidth / 40)), gp.avgCb, gp.avgCr, gp.avgY, upper);
  if (!useGoalColor || (abs(gp.avgCb - expectedGoalCb) < maxCbDiffOnGoal && abs(gp.avgCr - expectedGoalCr) < maxCrDiffOnGoal))
  {
    ...
    if (!scanForGreen(gp.endInImage, upper))
      return false;
    LINE("module:CLIPGoalPerceptor:scanForGoalPostBottom", gp.startInImage.x() + gp.topWidth * 0.5f, gp.startInImage.y(), gp.endInImage.x() + gp.bottomWidth * 0.5f, gp.endInImage.y(), 3, Drawings::solidPen, ColorRGBA::yellow);
    ...
    for (unsigned i = 0; i < lineVector.size(); i++)
    {
      ...
      if (Geometry::checkIntersectionOfLines(fieldLine.base, fieldLine.base + fieldLine.direction, goalLine.base, goalLine.base + goalLine.direction)
          && Geometry::getIntersectionOfLines(fieldLine, goalLine, intersection) && intersection.y() < gp.endInImage.y() && intersection.y() > 5) // not out of image
      {
        CROSS("module:CLIPGoalPerceptor:scanForGoalPostBottom", intersection.x(), intersection.y(), 5, 2, Drawings::solidPen, ColorRGBA::blue);
...
bool CLIPGoalPerceptor2015::scanForGoalPostTop(GoalPost& gp, const bool& upper)
{
  ...
  if (std::abs(direction.y()) < 1.f)
    return false;
  Vector2f startPoint(gp.startInImage.x() + gp.topWidth * 0.5f, gp.startInImage.y());
  int length = std::abs(scanSameColor(startPoint, direction, (int)(1 + (gp.topWidth / 40)), gp.avgCb, gp.avgCr, gp.avgY, upper));
...
CLIPGoalPercept::GoalPost::GoalPostSide CLIPGoalPerceptor2015::scanForGoalPostSide(const GoalPost& gp, const bool& upper)
{
  if (!gp.foundTop)
  ...
  Vector2i startPoint((int)(gp.startInImage.x() + 0.5f * gp.topWidth), (int)(gp.startInImage.y() - 0.5f * gp.topWidth));
...
bool CLIPGoalPerceptor2015::verifyBySize(GoalPost& gp, const bool useWorldModel, const bool& upper)
{
  ...
  if (useWorldModel) // TODO: check for head position (isPostVisible..)
  {
    ...
    if (std::abs(Transformation::fieldToRobot(theRobotPose, closePost).angle() - theJointSensorData.angles[Joints::headYaw]) < 0.6f)
    {
    ...
    else if (std::abs(Transformation::fieldToRobot(theRobotPose, Vector2f(closePost.x(), -closePost.y())).angle() - theJointSensorData.angles[Joints::headYaw]) < 0.6f)
    {
    ...
    if (std::abs(Transformation::fieldToRobot(theRobotPose, farPost).angle() - theJointSensorData.angles[Joints::headYaw]) < 0.6)
    {
    ...
    if (std::abs(Transformation::fieldToRobot(theRobotPose, Vector2f(farPost.x(), -farPost.y())).angle() - theJointSensorData.angles[Joints::headYaw]) < 0.6)
    {
...
bool CLIPGoalPerceptor2015::scanForGoalPostWidth(GoalPost& gp, const int yStep, const bool& upper)
{
  const int maxNumberOfScans = 5;
  float xStep = Vector2f((gp.endInImage.x() - gp.startInImage.x()), (gp.endInImage.y() - gp.startInImage.y())).normalize((float)yStep).x();
  ...
  {
    width[noScan] = scanForGoalPostWidthAt(Vector2i((int)xStart, yStart), gp.topWidth, startX[noScan], gp.avgCb, gp.avgCr, gp.avgY, upper);
    if (width[noScan] <= 0)
    {
      DEBUG_RESPONSE("debug drawing:module:CLIPGoalPerceptor:widthScan:upper")
      {
        if (upper)
          CROSS("module:CLIPGoalPerceptor:widthScan:upper", startX[noScan], yStart, yStep / 2, 3, Drawings::solidPen, ColorRGBA::black);
      }
      DEBUG_RESPONSE("debug drawing:module:CLIPGoalPerceptor:widthScan:lower")
      {
        if (!upper)
          CROSS("module:CLIPGoalPerceptor:widthScan:lower", startX[noScan], yStart, yStep / 2, 3, Drawings::solidPen, ColorRGBA::black);
      }
      fails++;
    }
    else
    {
      ...
      {
        if (upper)
          LINE("module:CLIPGoalPerceptor:widthScan:upper", startX[noScan], yStart, startX[noScan] + width[noScan], yStart, 3, Drawings::solidPen, ColorRGBA::yellow);
      }
      DEBUG_RESPONSE("debug drawing:module:CLIPGoalPerceptor:widthScan:lower")
      {
        if (!upper)
          LINE("module:CLIPGoalPerceptor:widthScan:lower", startX[noScan], yStart, startX[noScan] + width[noScan], yStart, 3, Drawings::solidPen, ColorRGBA::yellow);
      }
      validityPoints++;
...
float CLIPGoalPerceptor2015::scanForGoalPostWidthAt(const Vector2i& startPoint, const float& expectedWidth, int& realStartX, int& avgCb, int& avgCr, int& avgY, const bool& upper)
{
  ...
  for (; imageX < maxX; imageX++)
  {
    ...
    if ((std::abs(yDiff + lastYDiff) > 40 && std::abs(lastYDiff) <= 40) // TODO: params
        || std::abs(yDiffSum) > 40 || colorDiff > 40 || (lastYDiffBig && std::abs(yDiff + lastYDiff) < 20 && std::abs(yDiff) > 20))
    {
      yDiffSum = 0;
      if (!lastYDiffBig)
      {
    ...
    if (startX != baseX && endX == baseX)
    {
      ...
      if (colorSegments[segmentNo].length > 0 && (lastColorDiff > maxColorDiff || std::abs(yDiff + lastYDiff) > 50))
      {
  ...
  for (int i = 0; i <= segmentNo; i++)
  {
    if (colorSegments[i].length > minLength)
    {
      if (lastID < 0)
      {
        if (abs(colorSegments[i].avgCb - avgCb) < maxCbDiffOnGoal && abs(colorSegments[i].avgCr - avgCr) < maxCrDiffOnGoal)
        {
          lastID = i;
        }
        else
      ...
      {
        if (abs(colorSegments[i].avgCb - colorSegments[lastID].avgCb) < maxCbDiffOnGoal && abs(colorSegments[i].avgCr - colorSegments[lastID].avgCr) < maxCrDiffOnGoal
...
int CLIPGoalPerceptor2015::scanSameColor(const Vector2f& startPoint, const Vector2f& direction, const int& stepSize, int& avgCb, int& avgCr, int& avgY, const bool& upper)
{
  const Image& image = upper ? (Image&)theImageUpper : (Image&)theImage;
  const FieldColors& fieldColor = upper ? (FieldColorsUpper&)theFieldColorsUpper : theFieldColors;
  ...
  while (!image.isOutOfImage(checkPoint.x(), checkPoint.y(), 3) && wrongColorCount < 3 && wrongColorDiffCount < std::max(3, count / 5)) //TODO: param
  {
    p = image[(unsigned int)checkPoint.y()][(unsigned int)checkPoint.x()];
    if (std::abs(p.cb - avgCb) > maxCbDiffOnGoal || std::abs(p.cr - avgCr) > maxCrDiffOnGoal || (std::abs(p.y - avgY) > 60)
        || (useGoalColor && (std::abs(p.cb - expectedGoalCb) > maxCbDiffOnGoal || std::abs(p.cr - expectedGoalCr) > maxCrDiffOnGoal || ((abs(p.y - expectedGoalY) > 60) && std::abs(p.y - avgY) > 20)))
    ...
    else if (std::abs(p.cb - lastCb) > maxCbDiffOnGoal || std::abs(p.cr - lastCr) > maxCrDiffOnGoal)
      wrongColorDiffCount++;
    else
    {
      avgCbNew += p.cb;
...
int CLIPGoalPerceptor2015::checkForGoalPostColor(const Vector2i& startPoint, const Vector2i& direction, const int& width, const int& optCb, const int& optCr, const int& optY, const bool& upper)
{
  const Image& image = upper ? (Image&)theImageUpper : (Image&)theImage;
  ...
  while (!image.isOutOfImage(checkPoint.x(), checkPoint.y(), 3) && i < width && goalColorCount <= minGoalColorCount)
  {
    checkPoint += direction;
    i++;
    Image::Pixel p = image[(unsigned int)checkPoint.y()][(unsigned int)checkPoint.x()];
    if (abs(p.cr - optCr) < maxDiffCr && abs(p.cb - optCb) < maxDiffCb && (abs(p.y - optY) < maxDiffY || p.y > optY))
    {
      if (goalColorCount == 0)
      {
...
void CLIPGoalPerceptor2015::drawGoalPostCandidates()
{
  ...
  for (; gp != end; ++gp)
  {
    ...
    LINE("module:CLIPGoalPerceptor:goalPostCandidates", gp->startInImage.x(), gp->startInImage.y(), gp->startInImage.x() + gp->topWidth, gp->startInImage.y(), 3, Drawings::solidPen, baseColor);
    LINE("module:CLIPGoalPerceptor:goalPostCandidates", gp->startInImage.x() + gp->topWidth, gp->startInImage.y(), gp->endInImage.x(), gp->endInImage.y(), 3, Drawings::solidPen, baseColor);
    LINE("module:CLIPGoalPerceptor:goalPostCandidates", gp->endInImage.x(), gp->endInImage.y(), gp->endInImage.x() - gp->topWidth, gp->endInImage.y(), 3, Drawings::solidPen, baseColor);
    LINE("module:CLIPGoalPerceptor:goalPostCandidates", gp->endInImage.x() - gp->topWidth, gp->endInImage.y(), gp->startInImage.x(), gp->startInImage.y(), 3, Drawings::solidPen, baseColor);
    LINE("module:CLIPGoalPerceptor:goalPostCandidates", gp->endInImage.x(), gp->endInImage.y(), gp->startInImage.x(), gp->startInImage.y(), 2, Drawings::solidPen, baseColor);
    LINE("module:CLIPGoalPerceptor:goalPostCandidates", gp->startInImage.x() + gp->topWidth, gp->startInImage.y(), gp->endInImage.x() - gp->topWidth, gp->endInImage.y(), 2, Drawings::solidPen, baseColor);
  }
}
