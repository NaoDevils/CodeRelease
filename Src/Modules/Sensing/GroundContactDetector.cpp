...
void GroundContactDetector::update(GroundContactState& groundContactState)
{
  ...
  if (!ignoreSensors)
  {
    if (contact)
    {
      calibratedAccZValues.push_front(theJoinedIMUData.imuData[anglesource].acc.z());
      ...
      if ((useAngle && (angleNoise.x() > contactMaxAngleNoise || angleNoise.y() > contactMaxAngleNoise)) || (calibratedAccZValues.full() && calibratedAccZValues.average() > contactMaxAccZ))
      {
        /*
    ...
    else
    {
      ...
      const Vector2f gyro = theJoinedIMUData.imuData[anglesource].gyro.head<2>().cast<float>();
      const Vector3f acc = theJoinedIMUData.imuData[anglesource].acc.cast<float>();
      ...
      if (accNoises.full() && std::abs(accAverage.z() - Constants::g_1000) < 5.f && std::abs(accAverage.x()) < 5.f && std::abs(accAverage.y()) < 5.f && accNoise.x() < noContactMinAccNoise
          && accNoise.y() < noContactMinAccNoise && accNoise.z() < noContactMinAccNoise && gyroNoise.x() < noContactMinGyroNoise && gyroNoise.y() < noContactMinGyroNoise)
      {
        contact = true;
        useAngle = false;
        contactStartTime = theFrameInfo.time;
  ...
  expectedRotationInv = theRobotModel.limbs[Limbs::footLeft].translation.z() > theRobotModel.limbs[Limbs::footRight].translation.z()
      ? theRobotModel.limbs[Limbs::footLeft].rotation
      : theRobotModel.limbs[Limbs::footRight].rotation;
...
void GroundContactDetector::footContact(float& frequencyLeft, float& frequencyRight)
{
  ...
  float currentLeftAvg =
      (theFsrSensorData.left[FsrSensorData::fr] + theFsrSensorData.left[FsrSensorData::fl] + theFsrSensorData.left[FsrSensorData::br] + theFsrSensorData.left[FsrSensorData::bl]) / 4.f;
  float currentRightAvg =
      (theFsrSensorData.right[FsrSensorData::fr] + theFsrSensorData.right[FsrSensorData::fl] + theFsrSensorData.right[FsrSensorData::br] + theFsrSensorData.right[FsrSensorData::bl]) / 4.f;
