#include "DevilSmashStandardMessage.h"
...
namespace DevilSmash
{
  ...
  size_t RobotMap::sizeOf() const
  {
    // since the robot type will be serialized into 2 bits per robot (2 * 16 = 32)
    ASSERT(DS_STANDARD_MESSAGE_MAX_ROBOTS_IN_MAP <= 16);
      //,
  ...
  StandardMessage::StandardMessage()
    ...
    , ntpMessages()
  {
    ASSERT(DS_STANDARD_MESSAGE_STRUCT_VERSION == 5);
      //,
  ...
  size_t StandardMessage::sizeOfDSMessage() const
  {
    ASSERT(DS_STANDARD_MESSAGE_STRUCT_VERSION == 5);
    //,
    ...
    return sizeof(header) + sizeof(version) + sizeof(timestamp)
           ...
           + robotMap.sizeOf()
           + 1 // NTP receivers
           + ntpMessages.size() * NTPMessage::sizeOf();
    // clang-format on
  }
  ...
  bool StandardMessage::read(const void* data)
  {
    ...
    ASSERT((reinterpret_cast<const char*>(data) - reinterpret_cast<const char*>(begin)) ==
           static_cast<int>(sizeOfDSMessage()));
  ...
  void StandardMessage::write(void* data)
  {
    ...
    writeVal<uint16_t>(data, static_cast<uint16_t>(sizeOfDSMessage() - sizeof(header) - sizeof(version)));
    ...
    ASSERT(static_cast<unsigned int>(Role::MAX) <= 16);
    //,
    //              "Sending roles is not supported for more than 16 roles!");
    ASSERT(DS_STANDARD_MESSAGE_MAX_NUM_PLAYERS <= 7);
    //,
    ...
    // represents player 2, ...)
    ASSERT(DS_STANDARD_MESSAGE_MAX_NUM_PLAYERS <= 8);
    //,
