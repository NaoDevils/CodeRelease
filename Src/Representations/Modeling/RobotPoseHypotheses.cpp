...
void RobotPoseHypotheses::draw()
{
  ...
  COMPLEX_DRAWING("representation:RobotPoseHypotheses")
  {
    for (unsigned int i = 0; i < hypotheses.size(); ++i)
    {
      ...
      if (eigenvector1.x() == eigenvector1.x() && eigenvector1.y() == eigenvector1.y() && eigenvector2.x() == eigenvector2.x() && eigenvector2.y() == eigenvector2.y())
      {
        // Draw covariance ellipse:
        ASSERT((eigenvector1.y() != 0.0) || (eigenvector1.x() != 0.0));
        ELLIPSE("representation:RobotPoseHypotheses",
            pos,
            sqrt(eigenvalue1),
            sqrt(eigenvalue2),
            atan2(eigenvector1.y(), eigenvector1.x()),
            10,
            Drawings::solidPen,
            ColorRGBA(100, 100, 255, 170),
            Drawings::solidBrush,
            ColorRGBA(100, 100, 255, 170));
        LINE("representation:RobotPoseHypotheses", pos.x(), pos.y(), pos.x() + eigenvector1.x(), pos.y() + eigenvector1.y(), 10, Drawings::solidPen, ColorRGBA(0, 0, 0));
        LINE("representation:RobotPoseHypotheses", pos.x(), pos.y(), pos.x() - eigenvector1.x(), pos.y() - eigenvector1.y(), 10, Drawings::solidPen, ColorRGBA(0, 0, 0));
        LINE("representation:RobotPoseHypotheses", pos.x(), pos.y(), pos.x() + eigenvector2.x(), pos.y() + eigenvector2.y(), 10, Drawings::solidPen, ColorRGBA(0, 0, 0));
        LINE("representation:RobotPoseHypotheses", pos.x(), pos.y(), pos.x() - eigenvector2.x(), pos.y() - eigenvector2.y(), 10, Drawings::solidPen, ColorRGBA(0, 0, 0));
      }
      ...
      LINE("representation:RobotPoseHypotheses", pos.x(), pos.y(), pos.x() + dir.x(), pos.y() + dir.y(), 15, Drawings::solidPen, ColorRGBA(0, 0, 0));

      // Draw info on hypothesis
      DRAWTEXT("representation:RobotPoseHypotheses", pos.x() - 80, pos.y() - 180, 80, ColorRGBA(255, 255, 255), "pc: " << hypotheses[i].validity);
      DRAWTEXT("representation:RobotPoseHypotheses", pos.x() - 80, pos.y() - 260, 80, ColorRGBA(255, 255, 255), "sc: " << hypotheses[i].sideConfidenceState);
    }
