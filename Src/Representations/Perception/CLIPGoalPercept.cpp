  ...
  for (int i = 0; i < numberOfGoalPosts; i++)
  {
    if (goalPosts[i].fromUpper)
      ...
      QUADRANGLE("representation:CLIPGoalPercept:Image:Lower",
          ...
          Drawings::solidPen,
          ColorRGBA::yellow);
    Vector2i center = (goalPosts[i].topInImage + goalPosts[i].bottomInImage) / 2;

    if (goalPosts[i].fromUpper)
      CIRCLE("representation:CLIPGoalPercept:Image:Upper", center.x(), center.y(), 5, 2, Drawings::solidPen, ColorRGBA::gray, Drawings::noBrush, ColorRGBA::gray);
    else
      CIRCLE("representation:CLIPGoalPercept:Image:Lower", center.x(), center.y(), 5, 2, Drawings::solidPen, ColorRGBA::gray, Drawings::noBrush, ColorRGBA::gray);
    int factor = -1;
    ...
    {
      if (goalPosts[i].fromUpper)
        ARROW("representation:CLIPGoalPercept:Image:Upper", center.x(), center.y(), center.x() + factor * 30, center.y(), 2, Drawings::solidPen, ColorRGBA::white);
      else
        ARROW("representation:CLIPGoalPercept:Image:Lower", center.x(), center.y(), center.x() + factor * 30, center.y(), 2, Drawings::solidPen, ColorRGBA::white);
    }


    LINE("representation:CLIPGoalPercept:Field", 0, 0, goalPosts[i].locationOnField.x(), goalPosts[i].locationOnField.y(), 10, (goalPosts[i].bottomFound ? Drawings::dottedPen : Drawings::solidPen), ColorRGBA::yellow);
    CIRCLE("representation:CLIPGoalPercept:Field", goalPosts[i].locationOnField.x(), goalPosts[i].locationOnField.y(), 60, 10, Drawings::solidPen, ColorRGBA::white, Drawings::solidBrush, ColorRGBA::yellow);
