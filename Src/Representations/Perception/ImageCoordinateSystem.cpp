...
void ImageCoordinateSystem::draw() const
{
  ...
  ARROW("horizon", origin.x(), origin.y(), origin.x() + rotation(0, 0) * 50, origin.y() + rotation(1, 0) * 50, 0, Drawings::solidPen, ColorRGBA::red);
  ARROW("horizon", origin.x(), origin.y(), origin.x() + rotation(0, 1) * 50, origin.y() + rotation(1, 1) * 50, 0, Drawings::solidPen, ColorRGBA::red);
}
