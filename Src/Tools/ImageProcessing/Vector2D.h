...
public:
   ...
   */
  stdVector2D(int width, int height) : width(width), height(height) { this->resize(width * height); }
   ...
   * @return Value at (x,y)
   */
  const T& operator()(int x, int y) const { return this->operator[](x + y * width); }
   ...
   * @return Value at (x,y)
   */
  T& operator()(int x, int y) { return this->operator[](x + y * width); }
