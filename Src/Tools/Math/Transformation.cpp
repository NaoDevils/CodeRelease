...
bool Transformation::imageToRobot(const Vector2f& pointInImage, const CameraMatrix& cameraMatrix, const CameraInfo& cameraInfo, Vector2f& relativePosition)
{
  return imageToRobot(pointInImage.x(), pointInImage.y(), cameraMatrix, cameraInfo, relativePosition);
}

bool Transformation::imageToRobot(const float x, const float y, const CameraMatrix& cameraMatrix, const CameraInfo& cameraInfo, Vector2f& relativePosition)
{
  const float xFactor = cameraInfo.focalLengthInv;
  const float yFactor = cameraInfo.focalLengthInv;
  const Vector3f vectorToCenter(1.f, (cameraInfo.opticalCenter.x() - x) * xFactor, (cameraInfo.opticalCenter.y() - y) * yFactor);
...
bool Transformation::imageToRobot(const int x, const int y, const CameraMatrix& cameraMatrix, const CameraInfo& cameraInfo, Vector2f& relativePosition)
{
...
bool Transformation::imageToRobot(const int x, const int y, const CameraMatrix& cameraMatrix, const CameraInfo& cameraInfo, Vector2i& relativePosition)
{
  Vector2f pointFloat;
...
bool Transformation::imageToRobot(const Vector2i& pointInImage, const CameraMatrix& cameraMatrix, const CameraInfo& cameraInfo, Vector2f& relativePosition)
{
  return imageToRobot(pointInImage.x(), pointInImage.y(), cameraMatrix, cameraInfo, relativePosition);
}

bool Transformation::imageToRobotHorizontalPlane(const Vector2f& pointInImage, float z, const CameraMatrix& cameraMatrix, const CameraInfo& cameraInfo, Vector2f& pointOnPlane)
{
  const float xFactor = cameraInfo.focalLengthInv;
  const float yFactor = cameraInfo.focalLengthInv;
  const Vector3f vectorToCenter(1.f, (cameraInfo.opticalCenter.x() - pointInImage.x()) * xFactor, (cameraInfo.opticalCenter.y() - pointInImage.y()) * yFactor);
  const Vector3f vectorToCenterWorld = cameraMatrix.rotation * vectorToCenter;
  const float a1 = cameraMatrix.translation.x();
  const float a2 = cameraMatrix.translation.y();
...
bool Transformation::robotToImage(const Vector3f& point, const CameraMatrix& cameraMatrix, const CameraInfo& cameraInfo, Vector2f& pointInImage)
{
...
bool Transformation::robotToImage(const Vector2f& point, const CameraMatrix& cameraMatrix, const CameraInfo& cameraInfo, Vector2f& pointInImage)
{
  const Vector3f point3D(point.x(), point.y(), 0.f);
  return robotToImage(point3D, cameraMatrix, cameraInfo, pointInImage);
}

bool Transformation::robotWithCameraRotationToImage(const Vector2f& point, const CameraMatrix& cameraMatrix, const CameraInfo& cameraInfo, Vector2f& pointInImage)
{
...
bool Transformation::imageToRobotWithCameraRotation(const Vector2i& pointInImage, const CameraMatrix& cameraMatrix, const CameraInfo& cameraInfo, Vector2f& relativePosition)
{
