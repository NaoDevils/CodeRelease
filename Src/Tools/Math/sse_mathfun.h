...
inline float sseInnerProduct(const float* vec_a_start, int size, const float* vec_b_start, float init)
{
  // simple non-sse implementation, sse to follow
  float dot = init;

  for (int i = 0; i < size; i++)
  {
    dot += vec_a_start[i] * vec_b_start[i];
  }
