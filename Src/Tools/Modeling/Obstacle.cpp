...
Obstacle::Obstacle(const Matrix2f& covariance, const Vector2f& center, Type type) : covariance(covariance), center(center), velocity(Vector2f::Zero()), type(type)
{
  float radius = 55.f;
  if (type != goalpost)
    radius = getRobotDepth();
  else if (Blackboard::getInstance().exists("FieldDimensions"))
    radius = Blackboard::get<FieldDimensions>().goalPostRadius;
...
void Obstacle::fusion2D(Obstacle& one, const Obstacle& other)
{
  ...
  {
    OUTPUT_ERROR("Call Florian: one's type "
        << one.type << " cov: " << one.covariance(0, 0) << " " << one.covariance(1, 1) << " other type " << other.type << " cov: " << other.covariance(0, 0) << " " << other.covariance(1, 1));
    return;
  }
