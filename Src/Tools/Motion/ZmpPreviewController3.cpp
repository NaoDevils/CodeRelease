...
Matrix3d ZmpPreviewController3::dare2(const Matrix3d& A, const Matrix3x2d& B, const Matrix3d& Q, const Matrix2d& R)
{
...
void ZmpPreviewController3::init(const double dt, const double comHeight, const unsigned numPreviews, const double R, const double Qx, const double Qe, const Vector3d& QlDiag, const Vector2d& R0Diag)
{
  //see: writeParamNG.m in nao devils code release 2011
  const double zH = comHeight;
  const unsigned N = numPreviews;
  A0 << 1, dt, 0, Constants::g / zH * dt, 1, -Constants::g / zH * dt, 0, 0, 1;
...
double ZmpPreviewController3::step(const double currentZmp, const double currentCom, const std::deque<float>& zmpPreviews, const bool useFeedback, const Vector2f& feedbackFactor, const bool useFeedbackFactor)
{
  ASSERT(initialized);
