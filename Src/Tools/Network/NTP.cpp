...
#else
      localId = static_cast<unsigned char>(random(255 - 30) + 30);
#endif
    }
...
unsigned NTP::getRemoteTimeInLocalTime(unsigned remoteTime, unsigned char remoteId, unsigned localTime) const
{
  if(remoteId < MAX_NUM_OF_NTP_CLIENTS)
  {
    if (localTime > remoteTime) {
      while (localTime - remoteTime > 86400000){
        remoteTime += 86400000;
      }
    } else {
      while (remoteTime - localTime > 86400000) {
        remoteTime -= 86400000;
      }
    }
    return remoteTime;
...
void NTP::setCurrentRemoteID(unsigned int remoteIp)
{
  currentRemoteId = remoteIp & 0xff;
}
