...
ParticleSwarm::ParticleSwarm(unsigned int particleCount, float velocityFactor, float bestPositionFactor, float globalBestPositionFactor, float randomPositionFactor)
    : particleCount(particleCount), velocityFactor(velocityFactor), bestPositionFactor(bestPositionFactor), globalBestPositionFactor(globalBestPositionFactor), randomPositionFactor(randomPositionFactor)
{
}
...
void ParticleSwarm::start()
{
  ...
  for (unsigned int i = 0; i < particles.size(); ++i)
  {
    ...
    for (unsigned int j = 0; j < dimensions.size(); ++j)
    {
      ...
      particle.position[j] = *dimension.variable + (random >= 0.5f ? (dimension.max - *dimension.variable) * (random - 0.5f) * 2.f : (dimension.min - *dimension.variable) * random * 2.f);
    }
...
void ParticleSwarm::updateParticle(Particle& particle)
{
  ...
  {
    particle.velocity[i] = velocityFactor * particle.velocity[i] + bestPositionFactor * randomFloat() * (particle.bestPosition[i] - particle.position[i])
