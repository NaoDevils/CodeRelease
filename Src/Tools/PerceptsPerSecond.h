 ...
 *                  should be tracked.
 */
template <unsigned duration = 1000>
class PerceptsPerSecond
{
private:
  ...
  typedef std::deque<pps_entry> pps_deque;
  /**
...
public:
  ...
  void addPercepts(const PerceptsPerSecond& source)
  {
    ...
    for (typename pps_deque::const_iterator it_s = source.ringBuffer.begin(); it_s != source.ringBuffer.end(); ++it_s) {
      //while (it != ringBuffer.end() && *it > *it_s) ++it;
      //ringBuffer.insert(it, *it_s);
      //++it;
   ...
   */
  size_t pps() const { return ringBuffer.size(); }
   ...
   * \return Percepts per seconds.
   */
  size_t pps(unsigned currentTimestamp)
  {
...
private:
  ...
  {
    // Search for the first timestamp smaller than the new one.
    typename pps_deque::iterator it = ringBuffer.begin();
    while (it != ringBuffer.end() && it->timestamp() > entry.timestamp()) ++it;
    ringBuffer.insert(it, entry);
    
    cleanUp(ringBuffer.front().timestamp());
  }
