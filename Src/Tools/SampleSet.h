...
public:
   ...
   * @return a pose
   */
  Pose2f toPose() const { return Pose2f(angle, translation); }
};
...
protected:
  const char* data;
  const char* dataOld;
  int numberOfSamples, sizeOfEntries;
...
public:
   ...
   */
  void link(SampleSetProxyBase& sampleSetProxy) const { sampleSetProxy.link((const char*)current, (const char*)other, num, sizeof(T)); }
