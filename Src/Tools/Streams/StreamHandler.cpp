...
StreamHandler::StreamHandler() : registering(false), registeringBase(false)
{
  basicTypeSpecification[typeid(double).name()];
  basicTypeSpecification[typeid(uint64_t).name()];
...
Out& operator<<(Out& out, const StreamHandler& streamHandler)
{
  if (streamHandler.stringTable.empty())
  {
    ...
    for (StreamHandler::BasicTypeSpecification::const_iterator basicTypeIter = streamHandler.basicTypeSpecification.begin(); basicTypeIter != streamHandler.basicTypeSpecification.end(); ++basicTypeIter)
      out << Streaming::demangle(basicTypeIter->first);

    // specification
    out << static_cast<unsigned>(streamHandler.specification.size());
    for (StreamHandler::Specification::const_iterator specificationIter = streamHandler.specification.begin(); specificationIter != streamHandler.specification.end(); ++specificationIter)
    {
      out << Streaming::demangle(specificationIter->first) << static_cast<unsigned>(specificationIter->second.size());
      for (std::vector<StreamHandler::TypeNamePair>::const_iterator typeNamePairIter = specificationIter->second.begin(); typeNamePairIter != specificationIter->second.end(); ++typeNamePairIter)
    ...
    for (StreamHandler::EnumSpecification::const_iterator enumSpecificationIter = streamHandler.enumSpecification.begin(); enumSpecificationIter != streamHandler.enumSpecification.end(); ++enumSpecificationIter)
    {
      out << Streaming::demangle(enumSpecificationIter->first) << static_cast<unsigned>(enumSpecificationIter->second.size());
      for (std::vector<const char*>::const_iterator enumElementsIter = enumSpecificationIter->second.begin(); enumElementsIter != enumSpecificationIter->second.end(); ++enumElementsIter)
...
{
  a.basicTypeSpecification.insert(b.basicTypeSpecification.begin(), b.basicTypeSpecification.end());
  a.specification.insert(b.specification.begin(), b.specification.end());
  a.enumSpecification.insert(b.enumSpecification.begin(), b.enumSpecification.end());
  return a;
}
...
bool StreamHandler::areSpecificationsForTypesCompatible(StreamHandler& other, std::string type, std::string otherType)
{
  ...
  else
  {
    ...
    if (i != specification.end())
    {
      ...
      return type == otherType || (type.find("string") != std::string::npos && otherType.find("string") != std::string::npos)
