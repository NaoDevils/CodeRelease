...
Team::Team()
    : players(), selectedPlayers(), name(""), number(0), port(0), colorOwn(0xFFFF00), colorOpp(0x000000),
      //colorOwn(QColor(255,255,0)),
      //colorOpp(QColor(0,0,0)),
      wlanConfig(""), logConfig(""), buildConfig(""), volume(100), micVolume(75), deployDevice("")
{
  init();
}

Team::Team(const std::string& name, unsigned short number)
    : players(), selectedPlayers(), name(name), number(number), port(0),
      //colorOwn(""),
      //colorOpp(""),
      colorOwn(0xFFFF00), colorOpp(0x000000), wlanConfig(""), logConfig(""), buildConfig(""), volume(100), micVolume(75), deployDevice("")
{
  init();
...
void Team::serialize(In* in, Out* out)
{
  ...
  if (in)
  {
    ...
    for (size_t i = 0; i < players.size(); ++i)
    {
      const auto player = robots.find(players[i]);
      if (player != robots.end())
        addPlayer(i % MAX_PLAYERS, i / MAX_PLAYERS, *player->second);
    }
  }
...
std::vector<RobotConfigDorsh*> Team::getSelectedPlayers() const
{
  ...
  for (std::map<RobotConfigDorsh*, bool>::const_iterator i = selectedPlayers.begin(); i != selectedPlayers.end(); ++i)
    if (i->second)
