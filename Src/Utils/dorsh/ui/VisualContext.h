...
public:
  struct Entry
  {
    ...
    Entry(Type type, const QString& text) : type(type), text(new QString(text)) {}

    Entry(VisualContext* context) : type(CONTEXT), context(context) {}
...
public:
  ...
  virtual void executeInContextReachable(Console* console, TeamSelector* teamSelector, const QString& cmdLine);
