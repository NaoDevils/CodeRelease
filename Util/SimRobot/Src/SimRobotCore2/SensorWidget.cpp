...
void SensorWidget::paintFloatArrayWithDescriptionsSensor()
{
  ...
  {
    float value = conversionToDegreesNeeded ? toDeg(data.floatArray[i]) : data.floatArray[i];
    std::stringstream valueStream;
    valueStream << std::fixed << std::setprecision(4) << value;
    painter.drawText(0, 20 * i, this->width(), 20, Qt::AlignLeft, " " + descriptions[i]);
    painter.drawText(0, 20 * i, this->width(), 20, Qt::AlignRight, tr((QString(valueStream.str().c_str()) + " " + unitForDisplay + " ").toUtf8().data()));
  }
}
