...
SimObjectRenderer::SimObjectRenderer(SimObject& simObject)
    : simObject(simObject), width(0), height(0), cameraMode(targetCam), defaultCameraPos(3.f, 6.f, 4.f), cameraPos(defaultCameraPos), cameraTarget(Vector3f::Zero()), fovy(40.f),
...
void SimObjectRenderer::draw()
{
  ...
  if (moving)
  {
    ...
    {
      y += now - movingLeftStartTime;
      movingLeftStartTime = now;
    }
    if (movingRightStartTime)
    {
      y -= now - movingRightStartTime;
      movingRightStartTime = now;
    }
    if (movingUpStartTime)
    {
      x += now - movingUpStartTime;
      movingUpStartTime = now;
    }
    if (movingDownStartTime)
    {
      x -= now - movingDownStartTime;
      movingDownStartTime = now;
    }


    double angle = std::atan2(cameraPos.y() - cameraTarget.y(), cameraPos.x() - cameraTarget.x());
    Vector3f move(static_cast<float>(std::cos(angle) * x - std::sin(angle) * y) * 0.0015f, static_cast<float>(std::sin(angle) * x + std::cos(angle) * y) * 0.0015f, 0.f);
    cameraPos -= move;
    cameraTarget -= move;
  ...
  if (&simObject != Simulation::simulation->scene && !(renderFlags & showAsGlobalView))
  {
    ...
    Pose3f pose(
        (Matrix3f() << transformation[0], transformation[4], transformation[8], transformation[1], transformation[5], transformation[9], transformation[2], transformation[6], transformation[10])
            .finished(),
...
bool SimObjectRenderer::intersectRayAndPlane(const Vector3f& point, const Vector3f& v, const Vector3f& plane, const Vector3f& n, Vector3f& intersection) const
{
  ...
  else // object control
  {
    ...
    else if (dragMode == applyDynamics)
    {
      ...
      if (intersectRayAndPlane(cameraPos, projectedClick - cameraPos, dragSelection->pose.translation, dragPlaneVector, currentPos))
      {
        ...
        {
          const Vector3f offset = currentPos - dragStartPos;
          const Vector3f force = offset * static_cast<float>(dragSelection->mass.mass) * 200.f;
