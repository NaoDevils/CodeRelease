...
{
  data.floatValue = (dJointGetType(joint->joint) == dJointTypeHinge ? static_cast<float>(dJointGetHingeAngle(joint->joint)) : static_cast<float>(dJointGetSliderPosition(joint->joint)))
      + (joint->axis->deflection ? joint->axis->deflection->offset : 0.f);
}
...
{
  data.floatValue =
      dJointGetType(joint->joint) == dJointTypeHinge ? static_cast<float>(dJointGetHingeParam(joint->joint, dParamVel)) : static_cast<float>(dJointGetSliderParam(joint->joint, dParamVel));
}
