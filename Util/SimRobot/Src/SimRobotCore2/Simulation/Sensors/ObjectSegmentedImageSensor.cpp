...
void ObjectSegmentedImageSensor::ObjectSegmentedImageSensorPort::updateValue()
{
  ...
  int j = 0;
  for (std::list<Body*>::const_iterator iter = Simulation::simulation->scene->bodies.begin(), end = Simulation::simulation->scene->bodies.end(); iter != end; ++iter, ++j)
...
bool ObjectSegmentedImageSensor::ObjectSegmentedImageSensorPort::renderCameraImages(SimRobotCore2::SensorPort** cameras, unsigned int count)
{
  ...
  for (unsigned int i = 0; i < count; ++i)
  {
    ...
    if (sensor && sensor->lastSimulationStep != Simulation::simulation->simulationStep && sensor->camera->imageWidth == imageWidth && sensor->camera->imageHeight == imageHeight)
      ++imagesOfCurrentSize;
  }
